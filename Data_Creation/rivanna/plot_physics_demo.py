...
def plot_ssrf_burn_and_trajectory(
    *,
    ...
    ax.plot(t, traj["iplus"][:n], color="tab:red", lw=1.4, label=r"$I_+$ burn")
    ax.plot(t, traj["iminus"][:n], color="tab:blue", lw=1.4, label=r"$I_-$ burn")
    ax.plot(t, traj["ps"][:n], color="black", lw=1.1, ls="--", alpha=0.8, label=r"$P_s$ burn")
    ax.plot(t, traj["iplus"][:n] - traj["iminus"][:n], color="tab:orange", lw=1.1, ls="--", alpha=0.8, label=r"$Q$")
    ax.axvline(t[np.argmax(traj["iplus"][:n] - traj["iminus"][:n])], color="green", ls=":", alpha=0.7, linewidth=2.0)
    ax.set_xlabel("Time step", fontsize=18)
    ax.tick_params(axis='both', which='major', labelsize=12)
    ax.set_ylabel("Intensity (a.u.)", fontsize=18)
    ax.tick_params(axis='both', which='major', labelsize=12)
    ax.set_title(f"Burn bin {burn_bin} at R={burn_R:.3f}", fontsize=18)
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)

    plt.savefig(out_dir / f"dulya_v2_ssrf_burn_trajectory_P{polarization:.2f}_bin{burn_bin:04d}.png")
    plt.close(fig)
    print(f"Saved {out_dir / f"dulya_v2_ssrf_burn_trajectory_P{polarization:.2f}_bin{burn_bin:04d}.png"}", flush=True)
    return out_dir / f"dulya_v2_ssrf_burn_trajectory_P{polarization:.2f}_bin{burn_bin:04d}.png"
