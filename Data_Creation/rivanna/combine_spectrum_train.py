...
def combine_spectrum_shards(
    ...
    *,
    ...
    if include_unmanip:
        ...
        unmanip_report = {"ok": True, "n_p": 0, "n_missing": int(num_bins)}
        eq_p = np.zeros(0, dtype=float)
        eq_ps = np.zeros((0, int(num_bins)), dtype=STORE_DTYPE)
        eq_ip = np.zeros((0, int(num_bins)), dtype=STORE_DTYPE)
        eq_im = np.zeros((0, int(num_bins)), dtype=STORE_DTYPE)
        missing_unmanip = list(range(int(num_bins)))
    ...
    if missing_unmanip and not strict and include_unmanip:
        print(
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--no-unmanip",
        action="store_true",
        help="Skip unmanipulated equilibrium rows (requires spectrum-mode shards with ps_full)",
    )
    return p
