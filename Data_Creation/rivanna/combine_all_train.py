        ...
        for key in COMBINED_KEYS:
            ...
            if key not in data.files:
                ...
                if key == "n_steps" and "step" in data.files:
                    out[key] = np.asarray(data["step"], dtype=float)
                    continue
                if key == "gamma_rf":
                    out[key] = np.zeros(int(np.asarray(data["ps"]).size), dtype=float)
                    continue
...
def _modality_to_combined(
    ...
    *,
    ...
    if source == SOURCE_SSRF and "gamma_rf" in arrays:
        ...
        gamma_rf = np.zeros(n, dtype=float)
...
def save_combined_bin(
    ...
    *,
    ...
    meta = {
        ...
        "fields": (
            "ps,q=raw I± sums; P,Q=CC-calibrated true polarizations at this bin; "
            "gamma_rf,n_steps=global ssRF params (0 for unmanip/AFP); "
...
def combine_all(
    ...
    *,
    ...
    required_shards = manipulation_shard_bins(num_bins=int(num_bins))
