...
def run_one_bin(
    ...
    *,
    ...
    ps_lo = np.full((n_p, t_len), np.nan, dtype=float)
    iplus_lo = np.full((n_p, t_len), np.nan, dtype=float)
    iminus_lo = np.full((n_p, t_len), np.nan, dtype=float)
    ps_hi = np.full((n_p, t_len), np.nan, dtype=float)
    iplus_hi = np.full((n_p, t_len), np.nan, dtype=float)
    iminus_hi = np.full((n_p, t_len), np.nan, dtype=float)
    ps_full = iplus_full = iminus_full = None
    if capture_spectrum:
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not is_manipulation_shard_bin(
        bin_idx, p_values, num_bins=int(args.num_bins), shape_params=shape
    ):
        print(
