...
def load_fit_params(path: Optional[Path] = None) -> Dict[str, Any]:
    path = Path(path) if path is not None else DEFAULT_FIT_PARAMS_PATH
    with path.open("r", encoding="utf-8") as f:
        return json.load(f)


def shape_params_from_fit(fit_blob: Optional[Dict[str, Any]] = None) -> Dict[str, float]:
