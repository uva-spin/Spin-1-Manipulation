"""
Per-bin ssRF burn trajectories for SLURM array jobs (Dulya-fit equilibrium).
...
every timestep, for each initial vector polarization. Trajectories are stored at
fit scale (``GenerateDulyaLineshape`` units).
...
commit_ssrf_bins_only = commit_touched_bins_only
...
def _traj_to_fit_scale(traj: dict, from_spin1: float) -> dict:
    """Map Spin1 trajectory observables back to Dulya fit scale."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    save_shard(result, out, extra_meta=shape_meta(shape))
    print(
