import math
from importlib import util
from pathlib import Path
import numpy as np
import pandas as pd
from scipy.special import wofz
_afp_path = Path(__file__).resolve().parent.parent / 'physics' / 'afp.py'
_afp_spec = util.spec_from_file_location('_spin1_afp', str(_afp_path))
_afp_mod = util.module_from_spec(_afp_spec)
_afp_spec.loader.exec_module(_afp_mod)
AFP = _afp_mod.AFP

class ssRFMapper:

    def __init__(self, f, sigma, gamma, x0, amp):
        self.f = f
        self.sigma = sigma
        self.gamma = gamma
        self.x0 = x0
        self.amp = amp
        self.center_freq = 0

    def compute_lookup_tables(self, df):
        self.signal_to_iplus_lookup = {}
        self.signal_to_iminus_lookup = {}
        for freq_idx in range(len(self.f)):
            ps_values_at_freq = []
            iminus_values_at_freq = []
            iplus_values_at_freq = []
            row_indices = []
            for (row_idx, (_, ps_array, iminus_array, iplus_array)) in enumerate(zip(df['P'], df['Ps'], df['Iminus'], df['Iplus'])):
                ps_values_at_freq.append(ps_array[freq_idx])
                iminus_values_at_freq.append(iminus_array[freq_idx])
                iplus_values_at_freq.append(iplus_array[freq_idx])
                row_indices.append(row_idx)
            ps_values = np.array(ps_values_at_freq)
            iminus_values = np.array(iminus_values_at_freq)
            iplus_values = np.array(iplus_values_at_freq)
            sort_indices = np.argsort(ps_values)
            sorted_ps = ps_values[sort_indices]
            sorted_iminus = iminus_values[sort_indices]
            sorted_iplus = iplus_values[sort_indices]
            self.signal_to_iplus_lookup[freq_idx] = {'ps_values': sorted_ps, 'iplus_values': sorted_iplus, 'row_indices': np.array(row_indices)[sort_indices]}
            self.signal_to_iminus_lookup[freq_idx] = {'ps_values': sorted_ps, 'iminus_values': sorted_iminus, 'row_indices': np.array(row_indices)[sort_indices]}

    def map_signal_to_iplus_iminus(self, signal):
        """
        Map a signal (Ps values at each frequency) to corresponding IPlus and IMinus values.
        This is used to get the "true" IPlus and IMinus after shifting the signal.
        
        Args:
            signal: Array of shape (500,) containing Ps values at each frequency bin
            
        Returns:
            Tuple of (iplus_array, iminus_array), each of shape (500,)
        """
        (iplus_results, iminus_results) = self.map_signal_fast(signal, np.arange(len(signal)))
        return (iplus_results, iminus_results)

    def map_signal_fast(self, signal, indices):
        iplus_results = []
        iminus_results = []
        for (freq_idx, signal_value) in enumerate(signal):
            iplus_lookup = self.signal_to_iplus_lookup[freq_idx]
            iminus_lookup = self.signal_to_iminus_lookup[freq_idx]
            ps_values = iplus_lookup['ps_values']
            iplus_values = iplus_lookup['iplus_values']
            iminus_values = iminus_lookup['iminus_values']

            def find_nearest_index(array, value):
                idx = np.searchsorted(array, value, side='left')
                if idx > 0 and (idx == len(array) or math.fabs(value - array[idx - 1]) < math.fabs(value - array[idx])):
                    return idx - 1
                else:
                    return idx
            nearest_idx = find_nearest_index(ps_values, signal_value)
            closest_iplus = iplus_values[nearest_idx]
            closest_iminus = iminus_values[nearest_idx]
            iplus_results.append(closest_iplus)
            iminus_results.append(closest_iminus)
        iplus_results = np.array(iplus_results)
        iminus_results = np.array(iminus_results)
        return (iplus_results, iminus_results)

    def _voigt_profile(self, x, x0):
        """
        Calculate discretized Voigt profile.
        
        Args:
            x: Frequency array (bin centers)
            x0: Center frequency
            
        Returns:
            Voigt profile values at each bin
        """
        x_norm = (x - x0) / (self.sigma * np.sqrt(2))
        z = x_norm + 1j * (self.gamma / (self.sigma * np.sqrt(2)))
        voigt = np.real(wofz(z)) / (self.sigma * np.sqrt(2 * np.pi))
        return voigt

    def _gaussian_profile(self, x, x0):
        """
        Calculate discretized Gaussian profile.
        
        Args:
            x: Frequency array (bin centers)
            x0: Center frequency
            
        Returns:
            Gaussian profile values at each bin
        """
        gaussian = np.exp(-0.5 * ((x - x0) / self.sigma) ** 2) / (self.sigma * np.sqrt(2 * np.pi))
        return gaussian

    def _discretize_profile(self, profile_type='voigt'):
        """
        Generate discretized burn profile across frequency bins.
        
        Args:
            profile_type: 'voigt' or 'gaussian'
            
        Returns:
            Discretized profile array of same length as self.f
        """
        if profile_type.lower() == 'voigt':
            profile = self._voigt_profile(self.f, self.x0)
        elif profile_type.lower() == 'gaussian':
            profile = self._gaussian_profile(self.f, self.x0)
        else:
            raise ValueError(f"Unknown profile_type: {profile_type}. Use 'voigt' or 'gaussian'")
        max_val = np.max(profile)
        if max_val > 0:
            profile = profile / max_val
        profile = profile * self.amp
        return profile

    @staticmethod
    def _orient_burn_toward_zero(reference_ps, burn_values):
        """Hole-burn direction: reduce |Ps| using sign at reference Ps."""
        reference_ps = np.asarray(reference_ps)
        burn_values = np.asarray(burn_values)
        oriented = burn_values.copy()
        oriented[reference_ps < 0] = np.abs(oriented[reference_ps < 0])
        oriented[reference_ps > 0] = -np.abs(oriented[reference_ps > 0])
        oriented[reference_ps == 0] = 0.0
        return oriented

    @staticmethod
    def _clip_burn_to_preserve_sign(ps_values, burn_values):
        """Clip burns so Ps + burn never crosses the zero axis."""
        ps_values = np.asarray(ps_values)
        burn_values = np.asarray(burn_values)
        clipped = burn_values.copy()
        positive = ps_values > 0
        negative = ps_values < 0
        clipped[positive] = np.maximum(clipped[positive], -ps_values[positive])
        clipped[negative] = np.minimum(clipped[negative], -ps_values[negative])
        clipped[ps_values == 0] = 0.0
        return clipped

    @staticmethod
    def _clip_intensities_to_preserve_sign(updated, original, indices):
        """In-place: mapped I+/I- at burn bins cannot cross zero vs pre-burn values."""
        for (idx, orig) in zip(indices, original):
            val = updated[idx]
            if orig > 0:
                updated[idx] = max(val, 0.0)
            elif orig < 0:
                updated[idx] = min(val, 0.0)
            else:
                updated[idx] = 0.0

    def mirror_burn_over_center(self, burn_effect, burn_indices):
        mirrored_burn = np.zeros_like(self.f)
        mirrored_indices = []
        for (i, burn_idx) in enumerate(burn_indices):
            freq = self.f[burn_idx]
            mirrored_freq = 2 * self.center_freq - freq
            freq_diff = np.abs(self.f - mirrored_freq)
            mirrored_idx = np.argmin(freq_diff)
            if freq_diff[mirrored_idx] < (self.f[1] - self.f[0]) / 2:
                mirrored_burn[mirrored_idx] = burn_effect[i]
                mirrored_indices.append(mirrored_idx)
        return (mirrored_burn / 2, np.array(mirrored_indices))

    def apply_ssRF(self, ps, iplus, iminus, return_burn_info=False, profile_type='voigt'):
        """
        Apply ss-RF burn to ``ps`` and map to ``iplus`` / ``iminus`` (arrays updated in place).

        Returns ``(ps, iplus, iminus, rho_plus, rho_zero, rho_minus)`` where the last three
        are per-bin spin-1 populations from ``AFP.intensities_to_populations`` (same as
        ``physics/afp.py``), normalized so the three levels sum to 1 over all bins.

        If ``return_burn_info`` is True, appends a ``burn_info`` dict as the last element.
        """
        initial_ps = ps.copy()
        ssRF = self._discretize_profile(profile_type=profile_type)
        burn_threshold = self.amp * 0.001
        burn_indices = np.where(ssRF > burn_threshold)[0]
        ssRF_burn_values = ssRF[burn_indices]
        ps_at_burn = ps[burn_indices]
        ref_ps_at_burn = initial_ps[burn_indices]
        ssRF_burn_values = self._orient_burn_toward_zero(ref_ps_at_burn, ssRF_burn_values)
        clipped_burn_values = self._clip_burn_to_preserve_sign(ps_at_burn, ssRF_burn_values)
        original_iplus_at_burn = iplus[burn_indices].copy()
        original_iminus_at_burn = iminus[burn_indices].copy()
        ps[burn_indices] += clipped_burn_values
        (iplus_burn_effect, iminus_burn_effect) = self.map_signal_fast(ps, burn_indices)
        iplus[burn_indices] = iplus_burn_effect[burn_indices]
        iminus[burn_indices] = iminus_burn_effect[burn_indices]
        self._clip_intensities_to_preserve_sign(iplus, original_iplus_at_burn, burn_indices)
        self._clip_intensities_to_preserve_sign(iminus, original_iminus_at_burn, burn_indices)
        actual_iplus_change = np.abs(iplus[burn_indices] - original_iplus_at_burn)
        actual_iminus_change = np.abs(iminus[burn_indices] - original_iminus_at_burn)
        (mirrored_burn, mirrored_indices) = self.mirror_burn_over_center(clipped_burn_values, burn_indices)
        iplus_change_at_mirror = None
        iminus_change_at_mirror = None
        if np.any(mirrored_indices):
            mirrored_burn[mirrored_indices] *= -1.0
            burn_at_mirrored = mirrored_burn[mirrored_indices].copy()
            ps_at_mirror = ps[mirrored_indices].copy()
            clipped_mirror = self._clip_burn_to_preserve_sign(ps_at_mirror, burn_at_mirrored)
            original_iplus_at_mirror = iplus[mirrored_indices].copy()
            original_iminus_at_mirror = iminus[mirrored_indices].copy()
            ps[mirrored_indices] += clipped_mirror
            (iplus_mirror_effect, iminus_mirror_effect) = self.map_signal_fast(ps, mirrored_indices)
            iplus[mirrored_indices] = iplus_mirror_effect[mirrored_indices]
            iminus[mirrored_indices] = iminus_mirror_effect[mirrored_indices]
            self._clip_intensities_to_preserve_sign(iplus, original_iplus_at_mirror, mirrored_indices)
            self._clip_intensities_to_preserve_sign(iminus, original_iminus_at_mirror, mirrored_indices)
            iplus_change_at_mirror = np.abs(iplus[mirrored_indices] - original_iplus_at_mirror)
            iminus_change_at_mirror = np.abs(iminus[mirrored_indices] - original_iminus_at_mirror)
        (rho_plus, rho_zero, rho_minus) = AFP.intensities_to_populations(np.asarray(iplus), np.asarray(iminus))
        if return_burn_info:
            burn_info = {'burn_indices': burn_indices, 'mirror_indices': mirrored_indices if np.any(mirrored_indices) else None, 'iplus_change_at_burn': actual_iplus_change, 'iminus_change_at_burn': actual_iminus_change, 'iplus_change_at_mirror': iplus_change_at_mirror, 'iminus_change_at_mirror': iminus_change_at_mirror, 'rho_plus': rho_plus, 'rho_zero': rho_zero, 'rho_minus': rho_minus}
            return (ps, iplus, iminus, rho_plus, rho_zero, rho_minus, burn_info)
        return (ps, iplus, iminus, rho_plus, rho_zero, rho_minus)
