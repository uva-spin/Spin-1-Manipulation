...
class ssRFMapper:
    ...
    def map_signal_fast(self, signal: np.ndarray, indices: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for freq_idx, signal_value in enumerate(signal):
            ...
            # Find the index of the nearest Ps value at this frequency
            ...
            # Find the index of the closest Ps value
            nearest_idx = find_nearest_index(ps_values, signal_value)
            
            # Use that index to get the corresponding Iplus and Iminus at this frequency
    ...
    @staticmethod
    def _orient_burn_toward_zero(
        reference_ps: np.ndarray, burn_values: np.ndarray
    ) -> np.ndarray:
        """Hole-burn direction: reduce |Ps| using sign at reference Ps."""
        reference_ps = np.asarray(reference_ps, dtype=float)
        burn_values = np.asarray(burn_values, dtype=float)
    ...
    @staticmethod
    def _clip_burn_to_preserve_sign(
        ps_values: np.ndarray, burn_values: np.ndarray
    ) -> np.ndarray:
        """Clip burns so Ps + burn never crosses the zero axis."""
        ps_values = np.asarray(ps_values, dtype=float)
        burn_values = np.asarray(burn_values, dtype=float)
    ...
    @staticmethod
    def _clip_intensities_to_preserve_sign(
        updated: np.ndarray,
        original: np.ndarray,
        indices: np.ndarray,
    ) -> None:
    ...
    def apply_ssRF(self, ps: np.ndarray, iplus: np.ndarray, iminus: np.ndarray, return_burn_info: bool = False, profile_type: str = 'voigt') -> Tuple:
        ...
        burn_threshold = self.amp * 1e-3
        burn_indices = np.where(ssRF > burn_threshold)[0]
        ssRF_burn_values = ssRF[burn_indices]


        ### burn effect - reduce amplitude if burn reaches/crosses zero
        ...
        self._clip_intensities_to_preserve_sign(
            iplus, original_iplus_at_burn, burn_indices
        )
        self._clip_intensities_to_preserve_sign(
            iminus, original_iminus_at_burn, burn_indices
        )
        ...
        if np.any(mirrored_indices):
            ...
            clipped_mirror = self._clip_burn_to_preserve_sign(
            ...
            self._clip_intensities_to_preserve_sign(
                iplus, original_iplus_at_mirror, mirrored_indices
            )
            self._clip_intensities_to_preserve_sign(
                iminus, original_iminus_at_mirror, mirrored_indices
            )
