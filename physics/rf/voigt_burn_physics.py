...
class VoigtBurnPhysicsMixin:
    ...
    def _uses_legacy_discrete_rf(self):
        if getattr(self, '_rf_profile_frozen', False):
            return True
        return self.params.ssrf_subset_indices is not None

    def _uses_physical_voigt_rf(self):
        if self._uses_legacy_discrete_rf():
            return False
        p = self.params
        if getattr(p, 'use_physical_voigt_rf', False):
            return True
        return p.rf_gaussian_fwhm_R > 0.0 or p.rf_lorentzian_fwhm_R > 0.0

    def _rf_profile_key(self, center_R):
        p = self.params
        return (center_R, p.rf_gaussian_fwhm_R, p.rf_lorentzian_fwhm_R, str(p.rf_profile_normalization), p.rf_profile_quadrature_order, self.dR, len(self.Rplus), self.Rplus[0], self.Rplus[-1])

    def invalidate_rf_profile(self):
        self._rf_profile_cache_key = None
        self._rf_profile_cache = None

    def rf_profile_physical(self, center_R=None):
        ...
        p = self.params
        profile = bin_averaged_voigt(self.Rplus, center_R=center_R, bin_width_R=self.dR, gaussian_fwhm_R=p.rf_gaussian_fwhm_R, lorentzian_fwhm_R=p.rf_lorentzian_fwhm_R, normalization=str(p.rf_profile_normalization), quadrature_order=p.rf_profile_quadrature_order)
        self._rf_profile_cache_key = key
        self._rf_profile_cache = np.asarray(profile)
        return (self.Rplus.copy(), self._rf_profile_cache.copy())

    def rf_profile_arrays(self, center_R=None):
        """Profile weights at each packet's + and - transition frequencies."""
        (R, physical) = self.rf_profile_physical(center_R)
        if self.params.rf_gaussian_fwhm_R == 0.0 and self.params.rf_lorentzian_fwhm_R == 0.0:
            minus = np.zeros_like(physical)
            center = self.params.rf_burn_R if center_R is None else center_R
            if self.Rplus[0] <= -center <= self.Rplus[-1]:
                minus[np.argmin(np.abs(self.Rplus + center))] = 1.0
    ...
    def rf_profile_summary(self, center_R=None):
        ...
        return {'equivalent_width_R': np.sum(profile) * self.dR, 'equivalent_bins': np.sum(profile), 'bins_above_half': np.count_nonzero(profile >= 0.5 * peak) if peak > 0.0 else 0.0, 'bins_above_1pct': np.count_nonzero(profile >= threshold) if peak > 0.0 else 0.0, 'profile_peak': peak, 'approx_fwhm_R': approximate_voigt_fwhm(self.params.rf_gaussian_fwhm_R, self.params.rf_lorentzian_fwhm_R), 'backend': voigt_implementation_name(), 'normalization': str(self.params.rf_profile_normalization)}

    def rf_rate_fields(self, center_R=None, gamma_rf=None):
        if gamma_rf is None:
    ...
    def _rf_population_term(self, rf_on, center_R=None, gamma_rf=None):
    ...
    def local_diffusion_diagnostics(self, R=None, dnp_on=None):
        ...
        if kp is not None:
            ...
            if np.isfinite(Ip) and abs(Ip) > 1e-15:
                out['lambda_Iplus'] = slope / Ip
        if km is not None:
            slope = scale_obs * (net[km, ZERO] - net[km, MINUS])
            out['dIminus_diff_dt'] = slope
            Im = self.local_intensities(R)['Iminus']
            if np.isfinite(Im) and abs(Im) > 1e-15:
                out['lambda_Iminus'] = slope / Im
        return out

    def _physical_effective_local_rates(self, R=None):
        if R is None:
            R = self.params.rf_burn_R
        ...
        wp = cap['w_Iplus_R']
        wm = cap['w_Iminus_R']
        return {**cap, 'rf_profile_Iplus_R': vp, 'rf_profile_Iminus_R': vm, 'rf_profile_opposite_on_Iplus_packet': opposite_plus_packet, 'rf_profile_opposite_on_Iminus_packet': opposite_minus_packet, 'gamma_rf_Iplus_R': p.gamma_rf * wp * vp if np.isfinite(wp) and np.isfinite(vp) else np.nan, 'gamma_rf_Iminus_R': p.gamma_rf * wm * vm if np.isfinite(wm) and np.isfinite(vm) else np.nan, 'gamma_rf_opposite_on_Iplus_packet': p.gamma_rf * wp * opposite_plus_packet if np.isfinite(wp) and np.isfinite(opposite_plus_packet) else np.nan, 'gamma_rf_opposite_on_Iminus_packet': p.gamma_rf * wm * opposite_minus_packet if np.isfinite(wm) and np.isfinite(opposite_minus_packet) else np.nan, 'dnp_Iplus_R': p.dnp_rate * wp if np.isfinite(wp) else np.nan, 'dnp_Iminus_R': p.dnp_rate * wm if np.isfinite(wm) else np.nan, 'same_plus0_Iplus_R': p.d_same_plus0 * wp if np.isfinite(wp) else np.nan, 'same_0minus_Iminus_R': p.d_same_0minus * wm if np.isfinite(wm) else np.nan}
