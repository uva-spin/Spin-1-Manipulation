...
def test_capacity_weighting_identifies_horn_branch_at_same_physical_R():
    ...
    right = m.local_capacity_factors(0.9)
    left = m.local_capacity_factors(-0.9)
...
def test_capacity_weighted_rf_drains_horn_faster_than_overlap_branch():
    ...
    rates = m.effective_local_rates(0.9)
...
def test_capacity_weighted_dnp_has_larger_local_effective_rate_at_horn():
    m = Spin1Model(Spin1Params(p0=0.1, p_dnp_sat=0.58, dnp_rate=0.2, capacity_rate_power=1.0))
    rates = m.effective_local_rates(0.9)
    assert rates['dnp_Iplus_R'] > 3.0 * rates['dnp_Iminus_R']

def test_dnp_builds_toward_selected_saturation_without_overshoot():
    p = Spin1Params(p0=0.1, p_dnp_sat=0.58, dnp_enabled=True, dnp_rate=1.2, dt=0.0005, capacity_rate_power=0.0)
    m = Spin1Model(p)
    for _ in range(6000):
...
def test_no_dnp_internal_recovery_conserves_p_when_rf_off():
    ...
    assert abs(P_after_recovery - P_after_burn) < 5e-05
...
def test_direct_holes_and_mirror_peaks_signs_short_burn():
    m = Spin1Model(Spin1Params(rf_burn_R=-0.9, gamma_rf=2.0, dnp_enabled=False, d_same_plus0=0.0, d_same_0minus=0.0, d_spec_plus0=0.0, d_spec_0minus=0.0))
    m.params.rf_profile = np.full(m.params.n_bins, 2.0)
...
def test_afp_preserves_vector_polarization():
    ...
    m = Spin1Model(Spin1Params(p0=0.48, n_bins=500, r_min=-3.0, r_max=3.0, afp_subset_indices=AFP_SUBSET))
    m.load_from_physical_intensities(ip, im)
    P_before = m.level_populations()['P']
    m.afp_sweep()
    P_after = m.level_populations()['P']
    assert abs(P_after - P_before) < 0.2
...
def test_ssrf_mirror_burn_ratios_approximate_half():
    ...
    result = verify_rates_response(Iplus, Iminus, burn_idx=burn_idx, gamma_rf=2.0, dt=0.001, rtol=0.1, params=Spin1Params(capacity_rate_power=1.0, d_same_plus0=0.0, d_same_0minus=0.0, d_spec_plus0=0.0, d_spec_0minus=0.0), p0=0.48)
    assert result['magnitude_decreased']
    assert abs(result['ratios']['iplus_burn_over_iminus_mirror'] - 2.0) / 2.0 < 0.1
...
def test_voigt_multi_bin_uses_center_pair_capacity_weights():
    ...
    (_, ip, im) = GenerateVectorLineshape(0.48, f)
    burn_idx = np.argmin(np.abs(f - -0.92))
    params = Spin1Params(p0=0.48, n_bins=500, capacity_rate_power=1.0, gamma_rf=1.0, dt=0.0015, rf_burn_R=-0.92)
    ...
    profile = np.asarray(model.params.rf_profile)
    for _ in range(400):
...
def test_recovery_rates_use_capacity_weights():
    m = Spin1Model(_recovery_params(rf_burn_R=0.9, capacity_rate_power=1.0))
    rates = m.recovery_pathway_rates(0.9)
