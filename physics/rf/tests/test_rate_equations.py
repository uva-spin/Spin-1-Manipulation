...
def test_verify_rates_response_with_realistic_lineshape():
    ...
    assert result['ratios']['amp_burn_over_amp_mirror'] == pytest.approx(2.0, rel=0.1)

def test_solve_rate_equations_rf_only_preserves_ps_sign():
    (_, Iplus, Iminus) = _lineshape()
    burn_idx = 180
    (Iplus_new, Iminus_new, _, _, _) = solve_rate_equations(Iplus, Iminus, dt=0.001, gamma_rf=2.0, burn_idx=burn_idx, p0=0.45, rf_only=True)
...
def test_solve_rate_equations_full_dynamics_advances_state():
    (_, Iplus, Iminus) = _lineshape()
    burn_idx = 180
    params = Spin1Params(d_same_plus0=0.25, d_same_0minus=0.15, d_spec_plus0=1.5, d_spec_0minus=0.8, t2_width_R=0.05, dnp_enabled=False, steps=1, capacity_rate_power=1.0)
    (Iplus_new, Iminus_new, rho_plus, rho_zero, rho_minus) = solve_rate_equations(Iplus, Iminus, dt=0.001, gamma_rf=2.0, burn_idx=burn_idx, params=params, p0=0.45, full_dynamics=True)
