...
def _configure_single_bin_with_neighbors(model, *, gamma_rf=2.0):
    """Single-bin RF with the same demo recovery as physical Voigt."""
    burn_idx = model.burn_index(model.params.rf_burn_R)
    configure_single_bin_ssrf(model, burn_idx, gamma_rf)
    return burn_idx

def _setup_voigt_burn_model(*, single_bin, gamma_rf=2.0):
    model = create_voigt_burn_model(gamma_rf=gamma_rf)
    configure_voigt_burn_spectral_recovery(model)
    if single_bin:
        configure_single_bin_ssrf(model, model.burn_index(model.params.rf_burn_R), gamma_rf, apply_demo_recovery=False)
    return model

def _run_burn(model, *, steps):
    burn_idx = model.burn_index(model.params.rf_burn_R)
...
def _assert_integrated_mirror_half_ratios(ip0, im0, ip, im, burn_idx, *, rtol=0.12):
...
def test_voigt_is_one_common_physical_profile_for_both_transitions():
    m = Spin1Model(_physical_params(rf_burn_R=0.37, rf_gaussian_fwhm_R=0.08, rf_lorentzian_fwhm_R=0.04, rf_profile_normalization='center_bin'))
...
def test_voigt_rf_term_is_full_three_level_rate_equation():
    m = Spin1Model(_physical_params(p0=0.45, rf_burn_R=0.23, gamma_rf=1.9, rf_gaussian_fwhm_R=0.35, rf_lorentzian_fwhm_R=0.18, capacity_rate_power=0.7))
...
def test_internal_diffusion_conserves_every_packet_mass_and_total_vector_p():
    p = Spin1Params(diffusion_scale=5.0, cross_branch_ratio=0.4, orientation_corr_fraction=0.35)
    m = Spin1Model(p)
    ...
    assert abs(np.sum(dn[:, 0] - dn[:, 2])) < 1e-13
    assert abs(np.sum(dn)) < 1e-13

def _burned_model(burn_steps, R=0.4):
    p = _physical_params(p0=0.45, rf_burn_R=R, gamma_rf=6.0, rf_gaussian_fwhm_R=0.0, rf_lorentzian_fwhm_R=0.0, dt=0.0005)
    m = Spin1Model(p)
...
def test_isolated_ideal_bin_mirror_relation_emerges_from_population_equations():
    m = Spin1Model(_physical_params(rf_burn_R=0.4, gamma_rf=1.7, rf_gaussian_fwhm_R=0.0, rf_lorentzian_fwhm_R=0.0))
    (_, parts) = m.derivative(rf_on=True, dnp_on=False, breakdown=True)
    rf = parts['RF']
    assert np.isclose(rf['dIminus_minusR_dt'], -0.5 * rf['dIplus_R_dt'], rtol=1e-12, atol=1e-14)
    assert np.isclose(rf['dIplus_minusR_dt'], -0.5 * rf['dIminus_R_dt'], rtol=1e-12, atol=1e-14)
