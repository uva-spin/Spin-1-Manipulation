...
class PolarizationBurnLookupMapper(BurnLookupMapper):
    ...
    def compute_lookup_index(self):
        self.burn_index = build_burn_bin_index(self.burn_lookup_df, self.burn_bin_idx, polarization=self.polarization)
...
def _load_burn_lookup(burn_bin_idx, *, lookup_path):
...
def _find_brentq_bracket(func, amps):
    ...
    for ((a0, f0), (a1, f1)) in zip(values, values[1:]):
        ...
        if f0 * f1 < 0.0:
            return (a0, a1)
    raise ValueError(f'No sign change in residual; cannot bracket root. Sampled amps={amps}, residuals={[v for (_, v) in values]}')

def _find_burn_amp(polarization, burn_signal, burn_iplus, burn_iminus, *, target_signal):
    ...
    def _burned_signal_at_bin(burn_amp):
        (burned_signal, _, _) = mapper.apply_bin_burn(burn_signal.copy(), burn_iplus.copy(), burn_iminus.copy(), bin_idx=test_bin_idx, amp=burn_amp)
        return burned_signal[test_bin_idx]
    residual = lambda a: _burned_signal_at_bin(a) - target_signal
    (bracket_lo, bracket_hi) = _find_brentq_bracket(residual, np.linspace(0.0, 1.0, 41))
    return bracket_lo if bracket_lo == bracket_hi else brentq(residual, bracket_lo, bracket_hi)

def _apply_burn(polarization, burn_signal, burn_iplus, burn_iminus, burn_amp):
    burn_lookup_df = _load_burn_lookup(test_bin_idx, lookup_path=lookup_path)
    mapper = PolarizationBurnLookupMapper(f, polarization, test_bin_idx, burn_lookup_df)
    mapper.compute_lookup_index()
    return mapper.apply_bin_burn(burn_signal, burn_iplus, burn_iminus, bin_idx=test_bin_idx, amp=burn_amp)
matched_amp_p = _find_burn_amp(P, signal, iplus, iminus, target_signal=target_signal_amp)
matched_amp_ref = _find_burn_amp(P_REF, ref_signal, ref_iplus, ref_iminus, target_signal=target_signal_amp)
...
(new_signal, new_iplus, new_iminus) = _apply_burn(P, signal, iplus, iminus, matched_amp_p)
(ref_burn_signal, ref_burn_iplus, ref_burn_iminus) = _apply_burn(P_REF, ref_signal, ref_iplus, ref_iminus, matched_amp_ref)
