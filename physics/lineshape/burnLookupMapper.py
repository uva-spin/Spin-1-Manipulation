...
class BurnLookupMapper:
    ...
    def __init__(self, f, polarization, burn_bin_idx, burn_lookup_df, *, center_freq=0.0):
        self.f = np.asarray(f)
        self.polarization = polarization
        self.burn_bin_idx = burn_bin_idx
        self.center_freq = center_freq
        self.burn_lookup_df = burn_lookup_df
        self.burn_index = None

    def compute_lookup_index(self):
        """Build a Ps -> (Iplus, Iminus) index at the burn bin from all trajectories."""
        self.burn_index = build_burn_bin_index(self.burn_lookup_df, self.burn_bin_idx)
    ...
    def map_burn_bin(self, ps_at_burn):
        ...
        nearest_idx = self._map_ps(ps_values, ps_at_burn)
        return (self.burn_index['iplus_values'][nearest_idx], self.burn_index['iminus_values'][nearest_idx])
    ...
    def apply_bin_burn(self, ps, iplus, iminus, bin_idx, amp):
        """
        ...
        ps = np.asarray(ps).copy()
        iplus = np.asarray(iplus).copy()
        iminus = np.asarray(iminus).copy()
        initial_ps = ps.copy()
        ...
        clipped_burn_values = ssRFMapper._constrain_burn_no_zero_crossing(ps_at_burn, np.array([burn_val]))
        ...
        (iplus_burn, iminus_burn) = self.map_burn_bin(ps[burn_indices[0]])
        ...
        if np.any(mirrored_indices):
            ...
            burn_at_mirrored = ssRFMapper._constrain_burn_no_zero_crossing(ps_at_mirror, burn_at_mirrored)
