...
def _burned_ps_at_dt(polarization, burn_iplus, burn_iminus, dt):
...
def _apply_realtime_burn(polarization, burn_iplus, burn_iminus, dt):
...
def _trajectory_ps_values(polarization, burn_iplus, burn_iminus):
    """Ps at the burn bin along a ``burn_trajectory_realtime`` trajectory."""
    rows = burn_trajectory_realtime(burn_iplus, burn_iminus, test_bin_idx, polarization, f[test_bin_idx], config=trajectory_cfg)
    return np.asarray([row['ps_at_burn_bin'] for row in rows])
...
(new_signal, new_iplus, new_iminus) = _apply_realtime_burn(P, iplus, iminus, matched_dt_p)
(ref_burn_signal, ref_burn_iplus, ref_burn_iminus) = _apply_realtime_burn(P_REF, ref_iplus, ref_iminus, matched_dt_ref)
traj_ps_p = _trajectory_ps_values(P, iplus, iminus)
traj_ps_ref = _trajectory_ps_values(P_REF, ref_iplus, ref_iminus)
print(f'Burn dt (P={P * 100:.0f}% -> P_REF2={P_REF2 * 100:.0f}%): {matched_dt_p:.6e} ({matched_dt_p / trajectory_cfg.dt:.2f} base steps)')
print(f'Burn dt (P_REF={P_REF * 100:.0f}% -> P_REF2={P_REF2 * 100:.0f}%): {matched_dt_ref:.6e} ({matched_dt_ref / trajectory_cfg.dt:.2f} base steps)')
print(f'Target signal amplitude (P_REF2): {target_signal_amp:.6e}')
print(f'Trajectory Ps range (P): [{traj_ps_p[-1]:.6e}, {traj_ps_p[0]:.6e}], steps={len(traj_ps_p) - 1}')
print(f'Trajectory Ps range (P_REF): [{traj_ps_ref[-1]:.6e}, {traj_ps_ref[0]:.6e}], steps={len(traj_ps_ref) - 1}')
