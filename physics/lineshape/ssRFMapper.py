...
class ssRFMapper:
    ...
    @staticmethod
    def _map_ps(ps_values, target_ps):
        idx = np.searchsorted(ps_values, target_ps, side='left')
        if idx <= 0:
            return 0
    ...
    def map_signal(self, signal, indices):
        iplus_results = np.empty(len(signal))
        iminus_results = np.empty(len(signal))
    ...
    @staticmethod
    def _scale_burn_until_nonzero(ps_values, burn_values):
        ...
        for _ in range(200):
            ...
            reaches_or_crosses_zero = (ps_values > 0) & (ps_after_burn <= 0) | (ps_values < 0) & (ps_after_burn >= 0)
    ...
    @classmethod
    def _constrain_burn_no_zero_crossing(cls, ps_values, burn_values):
    ...
    def apply_bin_burn(self, ps, iplus, iminus, bin_idx, amp, return_burn_info=False):
        """
        ...
        iminus = iminus.copy()
        initial_ps = ps.copy()
        burn_indices = np.array([bin_idx], dtype=int)
        burn_val = amp
        ...
        clipped_burn_values = self._constrain_burn_no_zero_crossing(ps_at_burn, np.array([burn_val]))
        if np.all(clipped_burn_values == 0):
            if return_burn_info:
                burn_info = {'burn_indices': burn_indices, 'mirror_indices': None, 'clipped_burn_values': clipped_burn_values.copy(), 'clipped_mirror_burn_values': None, 'iplus_change_at_burn': np.zeros(len(burn_indices)), 'iminus_change_at_burn': np.zeros(len(burn_indices)), 'iplus_change_at_mirror': None, 'iminus_change_at_mirror': None}
        ...
        (mirrored_burn, mirrored_indices) = self.mirror_burn_over_center(np.array([burn_val]), burn_indices)
        mirrored_burn /= 2.0
        ...
        if np.any(mirrored_indices):
            ...
            burn_at_mirrored = self._constrain_burn_no_zero_crossing(ps_at_mirror, burn_at_mirrored)
        ...
        if return_burn_info:
            burn_info = {'burn_indices': burn_indices, 'mirror_indices': mirrored_indices if np.any(mirrored_indices) else None, 'clipped_burn_values': clipped_burn_values.copy(), 'clipped_mirror_burn_values': burn_at_mirrored.copy() if np.any(mirrored_indices) else None, 'iplus_change_at_burn': actual_iplus_change, 'iminus_change_at_burn': actual_iminus_change, 'iplus_change_at_mirror': iplus_change_at_mirror, 'iminus_change_at_mirror': iminus_change_at_mirror}
            return (ps, iplus, iminus, burn_info)
        return (ps, iplus, iminus)
    ...
    def apply_ssRF(self, ps, iplus, iminus, return_burn_info=False, profile_type='voigt'):
        ...
        if np.any(mirrored_indices):
            ...
            for (burn_idx, mirror_idx, ip_delta, im_delta) in zip(burn_indices, mirrored_indices, iplus_burn_delta, iminus_burn_delta):
            ...
            iminus_change_at_mirror = np.abs(iminus[mirrored_indices] - original_iminus_at_mirror)
        if return_burn_info:
            burn_info = {'burn_indices': burn_indices, 'mirror_indices': mirrored_indices if np.any(mirrored_indices) else None, 'iplus_change_at_burn': actual_iplus_change, 'iminus_change_at_burn': actual_iminus_change, 'iplus_change_at_mirror': iplus_change_at_mirror, 'iminus_change_at_mirror': iminus_change_at_mirror}
            return (ps, iplus, iminus, burn_info)
        return (ps, iplus, iminus)
