...
PARAM_ORDER = ('P', 'amp', 'center', 'cc', 'split', 'sigma', 'eta', 'xi', 'b0', 'b1', 'b2', 'b3')

def load_polysignal(csv_path=CSV_PATH):
    ...
    with csv_path.open('r') as f:
        for line in f:
            ...
            if len(parts) != N_COLS:
                continue
            rows.append([x for x in parts])
    raw = np.asarray(rows)
    event_numbers = raw[:, 0].astype(np.int64)
    signal = raw[:, 1:]
    signal_by_event = {event: signal[i] for (i, event) in enumerate(event_numbers)}
...
def absorption_components(x_eff, P, split, sigma, eta, *, wd=16.35, exact_intensity=False, nphi=64):
    """Single-site physical absorption (plus / minus / total)."""
    (plus, minus) = site_transition_components(x_eff, P, split, sigma, eta, wd=wd, exact_intensity=exact_intensity, nphi=nphi)
    return {'plus': plus, 'minus': minus, 'absorption': plus + minus}
...
def signal_model(x, P, amp, center, cc, split, sigma, eta, xi, b0, b1, b2, b3, *, wd=16.35, exact_intensity=False, nphi=64):
    """
    ...
    x = _as_1d_float(x)
    x_eff = cc * (x - center)
    comps = absorption_components(x_eff, P, split, sigma, eta, wd=wd, exact_intensity=exact_intensity, nphi=nphi)
    gain = qmeter_gain(x_eff, split, xi)
    background = polynomial_background(x, b0, b1, b2, b3)
    return amp * comps['absorption'] * gain + background

def component_curves(params, x, *, wd=16.35, exact_intensity=False, nphi=64):
    """Named physical components on the measured x-grid (amp applied)."""
    x = _as_1d_float(x)
    p = dict(params)
    x_eff = p['cc'] * (x - p['center'])
    comps = absorption_components(x_eff, p['P'], p['split'], p['sigma'], p['eta'], wd=wd, exact_intensity=exact_intensity, nphi=nphi)
    amp = p['amp']
...
def default_p0(*, p_guess=P_FIXED):
    return dict(P=p_guess, amp=1.0, center=0.0, cc=1.0, split=0.99, sigma=0.02, eta=0.05, xi=0.0, b0=0.0, b1=0.0, b2=0.0, b3=0.0)

def default_bounds():
    return dict(P=(0.0, 1.0), amp=(-np.inf, np.inf), center=(-0.3, 0.3), cc=(0.5, 1.5), split=(0.2, 2.0), sigma=(1e-05, 0.5), eta=(0.0, 0.5), xi=(-2.0, 2.0), b0=(-np.inf, np.inf), b1=(-np.inf, np.inf), b2=(-np.inf, np.inf), b3=(-np.inf, np.inf))

def seed_p0_from_data(x, y, *, p_guess=P_FIXED):
    ...
    if np.any(near):
        ...
        if np.any(left) and np.any(right):
            x_l = x[left][np.argmin(y[left])]
            x_r = x[right][np.argmin(y[right])]
...
def residual_metrics(x, y, params, *, wd=16.35, exact_intensity=True, nphi=64):
    ...
    y_model = signal_model(x, **{k: params[k] for k in PARAM_ORDER}, wd=wd, exact_intensity=exact_intensity, nphi=nphi)
    resid = _as_1d_float(y_model) - _as_1d_float(y)
    return {'rms': np.sqrt(np.mean(resid ** 2)), 'max_abs': np.max(np.abs(resid)), 'sse': np.sum(resid ** 2), 'residuals': resid, 'model': y_model}

def fit_signal(x, y, yerr=None, p0=None, bounds=None, fixed=None, *, wd=16.35, exact_intensity=True, nphi=64, loss='linear', f_scale=1.0, max_nfev=80000):
    """
    ...
    if 'amp' not in fixed:
        ...
        trial.update(fixed)
        trial['amp'] = 1.0
        seed = signal_model(x, **{k: trial[k] for k in PARAM_ORDER}, wd=wd, exact_intensity=exact_intensity, nphi=nphi)
        a0 = np.dot(y, seed) / (np.dot(seed, seed) + 1e-30)
        if abs(a0) < 1e-12:
            a0 = -1.0 if np.sum(y) < 0.0 else 1.0
        p0['amp'] = a0
...
def compute_transition_areas(x, fit, *, wd=16.35, exact_intensity=False, nphi=64):
    """Areas of physical plus/minus absorption (before Q-meter gain / baseline)."""
    x = _as_1d_float(x)
    curves = component_curves(fit.params, x, wd=wd, exact_intensity=exact_intensity, nphi=nphi)
    area_plus = _trapz(curves['plus'], x)
    area_minus = _trapz(curves['minus'], x)
    return {'area_plus': area_plus, 'area_minus': area_minus, 'area_total_physical': area_plus + area_minus, 'area_diff': area_plus - area_minus}

def plot_fit(x, y, fit, *, title='Single-site Dulya/Hamada deuteron fit', wd=16.35, exact_intensity=True, nphi=64, savepath=None, yerr=None):
    x = _as_1d_float(x)
    y = _as_1d_float(y)
    yerr = None if yerr is None else _as_1d_float(yerr)
    curves = component_curves(fit.params, x, wd=wd, exact_intensity=exact_intensity, nphi=nphi)
    metrics = residual_metrics(x, y, fit.params, wd=wd, exact_intensity=exact_intensity, nphi=nphi)
    residuals = metrics['residuals']
    areas = compute_transition_areas(x, fit, wd=wd, exact_intensity=exact_intensity, nphi=nphi)
    ...
    txt = f"P = {fit.params['P']:.5g}\nr = {p_to_r(fit.params['P']):.5g}\namp = {fit.params['amp']:.5g}\nsplit = {fit.params['split']:.5g}\nsigma = {fit.params['sigma']:.5g}\neta = {fit.params['eta']:.5g}\nxi = {fit.params['xi']:.5g}\nrms = {metrics['rms']:.3e}\nA+ = {areas['area_plus']:.5g}\nA- = {areas['area_minus']:.5g}"
    ax.text(0.98, 0.97, txt, transform=ax.transAxes, ha='right', va='top', fontsize=9, family='monospace', bbox={'boxstyle': 'round', 'facecolor': 'white', 'alpha': 0.85, 'edgecolor': '0.7'})
...
if __name__ == '__main__':
    ...
    bounds = default_bounds()
    fixed = {'cc': 1.0}
    fit = fit_signal(x, y, p0=p0, bounds=bounds, fixed=fixed, wd=16.35, exact_intensity=True, nphi=64, loss='linear', max_nfev=80000)
    metrics = residual_metrics(x, y, fit.params, wd=16.35, exact_intensity=True, nphi=64)
    ...
    shape_export = {'amp': fit.params['amp'], 'center': fit.params['center'], 'cc': fit.params['cc'], 'split': fit.params['split'], 'sigma': fit.params['sigma'], 'eta': fit.params['eta'], 'xi': fit.params['xi'], 'b0': fit.params['b0'], 'b1': fit.params['b1'], 'b2': fit.params['b2'], 'b3': fit.params['b3']}
    ...
    (areas, _fig, _metrics) = plot_fit(x, y, fit, title=f'Single-site Dulya fit — event {EVENT_NUMBER}\n', wd=16.35, exact_intensity=True, nphi=64, savepath=out_path)
