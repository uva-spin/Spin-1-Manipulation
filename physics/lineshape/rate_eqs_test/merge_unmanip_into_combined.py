...
ARRAY_KEYS = ('p0', 'step', 'center_bin', 'is_mirror', 'source', 'ps', 'iplus', 'iminus', 'amp')
...
def save_merged_bin(bin_idx, arrays, path):
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    source = np.asarray(arrays['source'], dtype=np.uint8)
    n_samples = np.asarray(arrays['ps']).size
    meta = {'bin_idx': bin_idx, 'n_samples': n_samples, 'n_ssrf': np.count_nonzero(source == SOURCE_SSRF), 'n_afp': np.count_nonzero(source == SOURCE_AFP), 'n_unmanip': np.count_nonzero(source == SOURCE_UNMANIP), 'source_codes': {'ssrf': SOURCE_SSRF, 'afp': SOURCE_AFP, 'unmanipulated': SOURCE_UNMANIP}, 'dataset': 'ssrf_afp_unmanip_train_bin', 'fields': 'ps,iplus,iminus,amp at this bin; center_bin; is_mirror; source (0=ssrf,1=afp,2=unmanip)'}
    tmp_path = path.with_name(f'.{path.stem}.{os.getpid()}.tmp.npz')
    try:
        np.savez_compressed(tmp_path, meta_json=np.asarray(json.dumps(meta)), bin_idx=np.asarray(bin_idx, dtype=np.int32), p0=_as_dtype('p0', arrays['p0']), step=_as_dtype('step', arrays['step']), center_bin=_as_dtype('center_bin', arrays['center_bin']), is_mirror=_as_dtype('is_mirror', arrays['is_mirror']), source=_as_dtype('source', arrays['source']), ps=_as_dtype('ps', arrays['ps']), iplus=_as_dtype('iplus', arrays['iplus']), iminus=_as_dtype('iminus', arrays['iminus']), amp=_as_dtype('amp', arrays['amp']))
...
def merge_bin(combined, unmanip):
    merged = {}
    for key in ARRAY_KEYS:
        merged[key] = np.concatenate([_as_dtype(key, combined[key]), _as_dtype(key, unmanip[key])])
    return merged

def merge_all(combined_dir, unmanip_dir, output_dir, *, num_bins=NUM_BINS, strict=True):
    ...
    if strict and (missing_combined or missing_unmanip):
        raise FileNotFoundError(f'Missing combined={len(missing_combined)} unmanip={len(missing_unmanip)}; first combined={missing_combined[:1]} first unmanip={missing_unmanip[:1]}')
    if missing_combined:
        print(f'WARNING: missing {len(missing_combined)} combined train bins', flush=True)
    if missing_unmanip:
        print(f'WARNING: missing {len(missing_unmanip)} unmanip bins', flush=True)
    return {'output_dir': str(output_dir), 'n_samples': samples_per_bin.sum(), 'samples_per_bin': samples_per_bin, 'ssrf_per_bin': counts['ssrf'], 'afp_per_bin': counts['afp'], 'unmanip_per_bin': counts['unmanip'], 'n_missing_combined': len(missing_combined), 'n_missing_unmanip': len(missing_unmanip), 'dataset': 'ssrf_afp_unmanip_train_bin'}

def build_arg_parser():
    p = argparse.ArgumentParser(description='Merge unmanipulated NPZs into combined ssRF/AFP per-bin train files')
...
def main(argv=None):
    ...
    print(f'Merging unmanip ({args.unmanip_dir}) into combined ({args.combined_dir}) -> {args.output_dir}', flush=True)
    result = merge_all(args.combined_dir, args.unmanip_dir, args.output_dir, num_bins=args.num_bins, strict=args.strict)
    print(f"Merged {result['n_samples']} samples -> {args.output_dir} (missing_combined={result['n_missing_combined']} missing_unmanip={result['n_missing_unmanip']})", flush=True)
