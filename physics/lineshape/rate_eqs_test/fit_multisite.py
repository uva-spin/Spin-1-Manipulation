...
def load_event_csv(path):
    ...
    if len(row) != N_BINS + 1:
        raise ValueError(f'Expected {N_BINS + 1} columns (event_id + {N_BINS} bins), got {len(row)}')
    event_id = row[0].strip()
    y = np.array([x for x in row[1:]])
    y_fit = -y
...
def powder_branch(R, A, eta, eps, nphi=64):
    ...
    phis = np.linspace(0.0, 0.5 * np.pi, nphi + 1)
    c2 = np.cos(2.0 * phis)
...
def transition_weights(R, P, split, wd, exact_intensity=False):
    ...
    vartheta = abs(split) / (3.0 * wd)
    plus = (r ** 2 - r ** (1.0 - 3.0 * vartheta * R)) / r ** (1.0 - vartheta * R)
    minus = (r ** (1.0 + 3.0 * vartheta * R) - 1.0) / r ** (1.0 + vartheta * R)
    return (plus, minus)

def site_transition_components(x_eff, P, split, sigma, eta, *, wd=16.35, exact_intensity=False, nphi=64):
    """
...
def butanol_absorption_components(x_eff, P, split_cd, split_od, sigma, eta_od, K, *, wd=16.35, eta_cd=0.0, exact_intensity=False, nphi=64):
    """
    ...
    x_eff = _as_1d_float(x_eff)
    K = K
    (cd_plus, cd_minus) = site_transition_components(x_eff, P, split_cd, sigma, eta_cd, wd=wd, exact_intensity=exact_intensity, nphi=nphi)
    (od_plus, od_minus) = site_transition_components(x_eff, P, split_od, sigma, eta_od, wd=wd, exact_intensity=exact_intensity, nphi=nphi)
    cd_plus *= 1.0 - K
    cd_minus *= 1.0 - K
    od_plus *= K
    od_minus *= K
    ...
    return {'cd_plus': cd_plus, 'cd_minus': cd_minus, 'cd_total': cd_plus + cd_minus, 'od_plus': od_plus, 'od_minus': od_minus, 'od_total': od_plus + od_minus, 'plus_total': plus_total, 'minus_total': minus_total, 'absorption': absorption}
...
def signal_model(x, P, amp, center, cc, split_cd, split_od, sigma, eta_od, K, xi, b0, b1, b2, b3, *, wd=16.35, eta_cd=0.0, exact_intensity=False, nphi=64):
    """
    ...
    x = _as_1d_float(x)
    x_eff = cc * (x - center)
    comps = butanol_absorption_components(x_eff, P, split_cd, split_od, sigma, eta_od, K, wd=wd, eta_cd=eta_cd, exact_intensity=exact_intensity, nphi=nphi)
    ...
    return amp * comps['absorption'] * gain + background

def component_curves(params, x, *, wd=16.35, eta_cd=0.0, exact_intensity=False, nphi=64):
    """
    ...
    x = _as_1d_float(x)
    p = dict(params)
    x_eff = p['cc'] * (x - p['center'])
    comps = butanol_absorption_components(x_eff, p['P'], p['split_cd'], p['split_od'], p['sigma'], p['eta_od'], p['K'], wd=wd, eta_cd=eta_cd, exact_intensity=exact_intensity, nphi=nphi)
    amp = p['amp']
    ...
    comps.update({'absorption_physical': comps['absorption'], 'qmeter_gain': gain, 'absorption_measured': absorption_measured, 'background': background, 'total': total, 'x_eff': x_eff})
    return comps
PARAM_ORDER = ('P', 'amp', 'center', 'cc', 'split_cd', 'split_od', 'sigma', 'eta_od', 'K', 'xi', 'b0', 'b1', 'b2', 'b3')
...
def _estimate_covariance(result):
    ...
    try:
        ...
        rss = np.sum(np.asarray(result.fun) ** 2)
        dof = max(n_data - n_par, 1)
        return jtj_inv * (rss / dof)
    except np.linalg.LinAlgError:
        return None

def fit_signal(x, y, yerr=None, p0=None, bounds=None, fixed=None, *, wd=16.35, eta_cd=0.0, exact_intensity=False, nphi=64, loss='soft_l1', f_scale=1.0, max_nfev=50000):
    """
    ...
    if p0 is None:
        p0 = dict(P=0.35, amp=1.0, center=0.0, cc=1.0, split_cd=0.9, split_od=1.15, sigma=0.05, eta_od=0.15, K=0.12, xi=0.0, b0=0.0, b1=0.0, b2=0.0, b3=0.0)
    if bounds is None:
        bounds = dict(P=(-0.99, 0.99), amp=(-np.inf, np.inf), center=(-0.5, 0.5), cc=(0.5, 1.5), split_cd=(0.0001, np.inf), split_od=(0.0001, np.inf), sigma=(1e-05, np.inf), eta_od=(0.0, 0.95), K=(0.0, 1.0), xi=(-0.5, 0.5), b0=(-np.inf, np.inf), b1=(-np.inf, np.inf), b2=(-np.inf, np.inf), b3=(-np.inf, np.inf))
    ...
    p0_vec = np.array([p0[name] for name in free_names])
    lb = np.array([bounds[name][0] for name in free_names])
    ub = np.array([bounds[name][1] for name in free_names])

    def residuals(p_free):
        params = _merge_params(p_free, PARAM_ORDER, p0, fixed)
        y_model = signal_model(x, **params, wd=wd, eta_cd=eta_cd, exact_intensity=exact_intensity, nphi=nphi)
        return (y_model - y) / sigma
    result = least_squares(residuals, p0_vec, bounds=(lb, ub), loss=loss, f_scale=f_scale, max_nfev=max_nfev)
...
def fit_clean_then_full(x, y, yerr, p0_full, bounds_full, *, clean_window=None, fixed_main=None, fixed_full=None, wd=16.35, eta_cd=0.0, exact_intensity=False, nphi=64, loss='soft_l1', f_scale=1.0, max_nfev=50000):
    """
    ...
    if clean_window is None:
        ...
        (lo, hi) = clean_window
        mask = (x >= lo) & (x <= hi)
    default_fixed_main = {'K': 0.0, 'eta_od': p0_full['eta_od'], 'split_od': p0_full['split_od'], 'xi': 0.0}
    if fixed_main is not None:
        default_fixed_main.update(dict(fixed_main))
    main_fit = fit_signal(x[mask], y[mask], None if yerr is None else yerr[mask], p0=p0_full, bounds=bounds_full, fixed=default_fixed_main, wd=wd, eta_cd=eta_cd, exact_intensity=exact_intensity, nphi=nphi, loss=loss, f_scale=f_scale, max_nfev=max_nfev)
    seeded = dict(p0_full)
    seeded.update(main_fit.params)
    full_fit = fit_signal(x, y, yerr, p0=seeded, bounds=bounds_full, fixed=fixed_full, wd=wd, eta_cd=eta_cd, exact_intensity=exact_intensity, nphi=nphi, loss=loss, f_scale=f_scale, max_nfev=max_nfev)
    return (main_fit, full_fit)

def compute_transition_areas(x, fit, *, wd=16.35, eta_cd=0.0, exact_intensity=False, nphi=64, n_mc=300, seed=123):
    """
    ...
    """
    x = _as_1d_float(x)
    curves = component_curves(fit.params, x, wd=wd, eta_cd=eta_cd, exact_intensity=exact_intensity, nphi=nphi)
    out = {'area_cd_plus': _trapz(curves['cd_plus'], x), 'area_cd_minus': _trapz(curves['cd_minus'], x), 'area_od_plus': _trapz(curves['od_plus'], x), 'area_od_minus': _trapz(curves['od_minus'], x)}
    ...
    try:
        if not np.all(np.isfinite(fit.pcov)):
            return out
        if np.linalg.cond(fit.pcov) > 1000000000000.0:
            return out
        L = np.linalg.cholesky(fit.pcov)
    except np.linalg.LinAlgError:
        return out
    base = np.array([fit.params[name] for name in free_names])
    ...
    for draw in draws:
        ...
        for (name, value) in zip(free_names, draw):
            trial[name] = value
        trial['P'] = np.clip(trial['P'], -0.999, 0.999)
        trial['cc'] = np.clip(trial['cc'], 1e-09, np.inf)
        trial['split_cd'] = np.clip(trial['split_cd'], 1e-09, np.inf)
        trial['split_od'] = np.clip(trial['split_od'], 1e-09, np.inf)
        trial['sigma'] = np.clip(trial['sigma'], 1e-09, np.inf)
        trial['eta_od'] = np.clip(trial['eta_od'], 0.0, 0.999)
        trial['K'] = np.clip(trial['K'], 0.0, 1.0)
        curves_i = component_curves(trial, x, wd=wd, eta_cd=eta_cd, exact_intensity=exact_intensity, nphi=nphi)
...
def plot_fit(x, y, yerr, fit, *, title='Multi-site Dulya/Hamada fit', wd=16.35, eta_cd=0.0, exact_intensity=False, nphi=64, savepath='multisite_dulya_fit.png'):
    """
    ...
    curves = component_curves(fit.params, dense, wd=wd, eta_cd=eta_cd, exact_intensity=exact_intensity, nphi=nphi)
    y_model_data = signal_model(x, **fit.params, wd=wd, eta_cd=eta_cd, exact_intensity=exact_intensity, nphi=nphi)
    residuals = y - y_model_data
    areas = compute_transition_areas(dense, fit, wd=wd, eta_cd=eta_cd, exact_intensity=exact_intensity, nphi=nphi)
    ...
    txt = f"P = {fit.params['P']:.5f}\nr = {p_to_r(fit.params['P']):.5f}\nsplit_cd = {fit.params['split_cd']:.5g}\nsplit_od = {fit.params['split_od']:.5g}\nsigma = {fit.params['sigma']:.5g}\neta_od = {fit.params['eta_od']:.5g}\nK = {fit.params['K']:.5g}\nA_plus = {areas['area_plus_total']:.5g}\nA_minus = {areas['area_minus_total']:.5g}\nA_plus - A_minus = {areas['area_diff_total']:.5g}"
    ax.text(0.02, 0.98, txt, transform=ax.transAxes, va='top', ha='left', fontsize=9, bbox=dict(boxstyle='round', facecolor='white', alpha=0.78, lw=0.5))
...
if __name__ == '__main__':
    ...
    x = x_full[m]
    y = y_full[m]
    yerr = np.full_like(x, 1.816364e-05)
    p0_full = dict(P=0.35, amp=0.9, center=0.0, cc=1.0, split_cd=0.88, split_od=1.12, sigma=0.05, eta_od=0.12, K=0.1, xi=0.0, b0=0.0, b1=0.0, b2=0.0, b3=0.0)
    bounds_full = dict(P=(0.0, 0.95), amp=(-np.inf, np.inf), center=(-0.2, 0.2), cc=(0.0, 1.2), split_cd=(0.0, 1.5), split_od=(0.0, 1.8), sigma=(0.01, 0.2), eta_od=(0.0, 0.5), K=(0.0, 0.4), xi=(-0.15, 0.15), b0=(-1.0, 1.0), b1=(-1.0, 1.0), b2=(-1.0, 1.0), b3=(-1.0, 1.0))
    fixed_full = {'cc': 1.0, 'xi': 0.0, 'b2': 0.0, 'b3': 0.0}
    clean_window = (-1.05, 1.05)
    (main_fit, full_fit) = fit_clean_then_full(x, y, yerr, p0_full, bounds_full, clean_window=clean_window, fixed_main={'cc': 1.0, 'b2': 0.0, 'b3': 0.0}, fixed_full=fixed_full, wd=16.35, eta_cd=0.0, exact_intensity=True, nphi=48, loss='soft_l1', f_scale=1.0, max_nfev=300)
    ...
    areas = plot_fit(x, y, yerr, full_fit, title='Physical multi-site fit: C-D + O-D composition', wd=16.35, eta_cd=0.0, exact_intensity=True, nphi=48, savepath='./multisite_fit_demo.png')
