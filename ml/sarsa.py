...
class SARSAAgent:
    """SARSA agent for sequential bin-wise ssRF burns on a tensor lineshape."""
    ...
    def __init__(
        ...
        self.q_table = np.full((n_states, n_actions), 0.0, dtype=np.float64)
...
def train(
    ...
    eps_start: float = 1.0,
    eps_end: float = 0.02,
) -> tuple[SARSAAgent, TensorBurnEnv, dict]:
    ...
    for ep in tqdm.tqdm(range(episodes), desc="Training SARSA agent"):
        ...
        for _ in range(config.max_burns):
            ...
            next_action = agent.select_action(
                next_state, explore=True, action_mask=next_mask
            )
            agent.update(
...
def plot_training_returns(returns: np.ndarray, output_path: Path) -> None:
    ...
    ax.set_title("SARSA: bin-wise ssRF burn policy")
    ax.legend()
...
def main() -> None:
    ...
    print("Training SARSA agent for tensor Q polarization...")
