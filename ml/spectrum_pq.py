...
def polarization_range_stats(pred_p, true_p, pred_q, true_q, *, pol_ref, bands=POL_ABS_BANDS, ref_name='abs_P'):
...
def print_range_stats_table(rows, *, title):
    ...
    for row in rows:
        if row['n'] == 0:
            continue
        print(f"{row['range']:>10} {row['n']:5d} {row['P_rpe_median']:10.3f} {row['P_rpe_mean']:10.3f} {row['P_rpe_std']:10.3f} {row['P_residual_mean']:11.5f} {row['P_residual_std']:10.5f} {row['Q_rpe_median']:10.3f} {row['Q_rpe_mean']:10.3f} {row['Q_rpe_std']:10.3f} {row['Q_residual_mean']:11.5f} {row['Q_residual_std']:10.5f}", flush=True)
...
def load_spectrum_pq_npz(path):
    ...
    features = np.concatenate([ps.reshape(n, num_bins), applied_power.reshape(n, 1), n_steps.reshape(n, 1)], axis=1).astype(np.float32, copy=False)
    out = {'spectra': spectra.astype(np.float32, copy=False), 'features': features, 'P_total': p_total, 'Q_total': q_total, 'applied_power': applied_power, 'n_steps': n_steps, 'num_bins': np.asarray(num_bins, dtype=np.int32)}
    out.update(optional)
    return out
...
def _standardize_fit(x):
    ...
    std = np.where(std < 1e-08, 1.0, std).astype(np.float32)
    return (mean, std)

def _apply_standardize(x, mean, std):
    return ((x - mean) / std).astype(np.float32, copy=False)

def prepare_datasets(arrays, *, val_frac=VAL_FRAC, test_frac=TEST_FRAC, seed=SEED):
    spectra = np.asarray(arrays['spectra'])
    ps = spectra[:, 0, :] + spectra[:, 1, :]
    ps += np.random.normal(0, 0.0001, size=ps.shape)
    applied_power = np.asarray(arrays['applied_power']).reshape(-1, 1)
    n_steps = np.asarray(arrays['n_steps']).reshape(-1, 1)
    ...
    (p_mean_s, p_std_s) = (p_m[0], p_s[0])
    (q_mean_s, q_std_s) = (q_m[0], q_s[0])
    ...
    def _pack(indices):
        ...
        return data.TensorDataset(torch.from_numpy(features_combined), torch.from_numpy(yp), torch.from_numpy(yq))
    stats = {'ps_mean': ps_mean, 'ps_std': ps_std, 'pwr_mean': pwr_mean, 'pwr_std': pwr_std, 'steps_mean': steps_mean, 'steps_std': steps_std, 'P_mean': p_mean_s, 'P_std': p_std_s, 'Q_mean': q_mean_s, 'Q_std': q_std_s, 'input_dim': ps.shape[1] + 2, 'n_train': train_idx.size, 'n_val': val_idx.size, 'n_test': test_idx.size, 'train_idx': train_idx, 'val_idx': val_idx, 'test_idx': test_idx, 'p0_test': arrays.get('p0', y_p)[test_idx]}
...
def save_best_checkpoint(path, *, model, stats, hidden_dims, best_val_loss, best_epoch):
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save({'model_state_dict': clone_state_dict(model), 'stats': _stats_for_checkpoint(stats), 'hidden_dims': tuple((h for h in hidden_dims)), 'best_val_loss': best_val_loss, 'best_epoch': best_epoch, 'input_dim': stats['input_dim']}, path)

def load_best_checkpoint(path, *, device=DEVICE):
    ckpt = torch.load(path, map_location=device, weights_only=False)
    hidden_dims = tuple((h for h in ckpt['hidden_dims']))
    input_dim = ckpt.get('input_dim', ckpt['stats']['input_dim'])
    model = SpectrumPQModel(input_dim=input_dim, hidden_dims=hidden_dims).to(device)
    model.load_state_dict(ckpt['model_state_dict'])
    return (model, ckpt)

def train_model(train_dataset, val_dataset, stats, *, hidden_dims, num_epochs, batch_size, learning_rate, patience, checkpoint_path=None, device=DEVICE):
    ...
    q_mean = stats['Q_mean']
    q_std = stats['Q_std']
    model = SpectrumPQModel(input_dim=train_dataset.tensors[0].shape[1], hidden_dims=hidden_dims).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for (x_v, y_p, y_q) in val_loader:
                ...
                rpe_p_sum += (torch.abs(pred_p_n - true_p_n) / (torch.abs(true_p_n) + REL_LOSS_EPS)).sum().item()
                rpe_q_sum += (torch.abs(pred_q_n - true_q_n) / (torch.abs(true_q_n) + REL_LOSS_EPS)).sum().item()
                n_val += y_p.numel()
        ...
        if best_state is None or avg_val < best_val - MIN_DELTA:
            ...
            if checkpoint_path is not None:
                save_best_checkpoint(checkpoint_path, model=model, stats=stats, hidden_dims=hidden_dims, best_val_loss=best_val, best_epoch=best_epoch)
        else:
            stale += 1
        if epoch % 10 == 0 or epoch == num_epochs - 1:
            lr = optimizer.param_groups[0]['lr']
            print(f'epoch {epoch:04d} | train {avg_train:.6f} | val {avg_val:.6f} | val RPE% P={rpe_p:.3f} Q={rpe_q:.3f} | lr {lr:.2e}', flush=True)
        if stale >= patience:
...
@torch.no_grad()
def predict_denormalized(model, dataset, stats, *, batch_size=1024, device=DEVICE):
...
@torch.no_grad()
def evaluate_model(model, dataset, stats, *, batch_size=1024, device=DEVICE):
    (pred_p_arr, pred_q_arr, true_p_arr, true_q_arr) = predict_denormalized(model, dataset, stats, batch_size=batch_size, device=device)
...
def save_plots(history, metrics, plots_dir):
    ...
    for (ax, label, true_key, pred_key, mae_key, r2_key) in ((axes[0], 'P_total', 'true_P', 'pred_P', 'P_mae', 'P_r2'), (axes[1], 'Q_total', 'true_Q', 'pred_Q', 'Q_mae', 'Q_r2')):
        ...
        ax.set_title(f'{label}: MAE={metrics[mae_key]:.4g}  R²={metrics[r2_key]:.4f}')
        ax.grid(True, alpha=0.3)
...
def _select_example_indices(n_test, n_examples, *, true_p, pred_p, true_q, pred_q, seed=SEED):
    n = n_test
    ...
    if picks.size < k:
        rng = np.random.default_rng(seed)
        extra = rng.choice(np.setdiff1d(np.arange(n), picks, assume_unique=False), size=k - picks.size, replace=False)
        picks = np.concatenate([picks, extra])
    return np.sort(picks[:k].astype(int))

def save_example_signal_plots(arrays, stats, metrics, plots_dir, *, n_examples=6, seed=SEED):
    plots_dir.mkdir(parents=True, exist_ok=True)
    ...
    if pred_p.size != test_idx.size:
        raise ValueError(f'metrics length {pred_p.size} != test_idx length {test_idx.size}')
    pick = _select_example_indices(test_idx.size, n_examples, true_p=true_p, pred_p=pred_p, true_q=true_q, pred_q=pred_q, seed=seed)
    applied = np.asarray(arrays['applied_power']).reshape(-1)
    n_steps_arr = np.asarray(arrays['n_steps']).reshape(-1)
    has_p0 = 'p0' in arrays
    p0_arr = np.asarray(arrays['p0']).reshape(-1) if has_p0 else None
    center = np.asarray(arrays['center_bin'], dtype=np.int64).reshape(-1) if 'center_bin' in arrays else None
    source = np.asarray(arrays['source'], dtype=np.int64).reshape(-1) if 'source' in arrays else None
    source_name = {0: 'ssRF', 1: 'AFP'}
    saved = []
    summary_rows = min(4, pick.size)
    if summary_rows > 0:
        (fig, axes) = plt.subplots(summary_rows, 2, figsize=(11.5, 3.2 * summary_rows), squeeze=False)
    else:
        fig = None
        axes = None
    for (panel_i, local_i) in enumerate(pick):
        ...
        tq = true_q[local_i]
        pq = pred_q[local_i]
        rpe_p = compute_rpe(np.array([pp]), np.array([tp]))[0]
        rpe_q = compute_rpe(np.array([pq]), np.array([tq]))[0]
        p0_v = p0_arr[gi] if p0_arr is not None else 'nan'
        power = applied[gi]
        steps = n_steps_arr[gi]
        bin_v = center[gi] if center is not None else -1
        src = source_name.get(source[gi], str(source[gi])) if source is not None else '?'
        (fig_e, (ax_s, ax_pq)) = plt.subplots(1, 2, figsize=(11.5, 4.2), gridspec_kw={'width_ratios': [2.2, 1.0]})
...
def main():
    ...
    parser.add_argument('--checkpoint', type=Path, default=None, help='Best-model checkpoint path (default: <out-dir>/spectrum_pq_best.pth)')
    ...
    (model, best_val, history) = train_model(train_ds, val_ds, stats, hidden_dims=HIDDEN_DIMS, num_epochs=args.epochs, batch_size=args.batch_size, learning_rate=args.lr, patience=args.patience, checkpoint_path=checkpoint_path)
