...
class LinearBinModel(nn.Module):
    ...
    def __init__(self, use_hidden=False, hidden_dim=1):
        super().__init__()
        if use_hidden:
            self.net = nn.Sequential(nn.Linear(1, hidden_dim, bias=True), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim, bias=True), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim, bias=True), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim, bias=True), nn.ReLU(), nn.Linear(hidden_dim, 2, bias=True))
        else:
...
class Combined500BinModel(nn.Module):
    ...
    def __init__(self, bin_models, stats):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for bin_idx in range(self.num_bins):
            ...
            pred_iplus[:, bin_idx] = iplus_norm * Iplus_std[bin_idx] + Iplus_mean[bin_idx]
            pred_iminus[:, bin_idx] = iminus_norm * Iminus_std[bin_idx] + Iminus_mean[bin_idx]
        return (pred_iplus, pred_iminus)

def train_single_bin(target_bin, Ps_train_norm, Iplus_train_norm, Iminus_train_norm, val_Ps_bin, val_Iplus_bin, val_Iminus_bin, Ps_mean, Ps_std, Iplus_mean, Iplus_std, Iminus_mean, Iminus_std, device, args, resume_checkpoint=None):
    ...
    bin_train_dataset = data.TensorDataset(train_Ps_bin, train_Iplus_bin, train_Iminus_bin)
    bin_val_dataset = data.TensorDataset(val_Ps_bin, val_Iplus_bin, val_Iminus_bin)
    bin_train_loader = data.DataLoader(bin_train_dataset, batch_size=args.batch_size, shuffle=True)
    bin_val_loader = data.DataLoader(bin_val_dataset, batch_size=args.batch_size, shuffle=False)
    model = LinearBinModel(use_hidden=args.use_hidden, hidden_dim=args.hidden_dim).to(device)
    if resume_checkpoint is not None and os.path.exists(resume_checkpoint):
        ckpt = torch.load(resume_checkpoint, map_location=device)
        model.load_state_dict(ckpt['model_state_dict'])
        print(f'  Bin {target_bin}: resumed from {resume_checkpoint}')
    optimizer = optim.AdamW(model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=args.lr_factor, patience=args.lr_patience, min_lr=args.lr_min)
    loss_fn = nn.L1Loss()
    best_val_loss = 'inf'
    ...
    for epoch in range(args.num_epochs):
        ...
        with torch.no_grad():
            for (X_val, y_iplus_val, y_iminus_val) in bin_val_loader:
                ...
                val_loss = loss_fn(pred_iplus, y_iplus_val) + loss_fn(pred_iminus, y_iminus_val)
                val_loss_sum += val_loss.item()
                val_batches += 1
        avg_val_loss = val_loss_sum / val_batches if val_batches > 0 else 'inf'
        ...
        if epoch % 100 == 0 or epoch == args.num_epochs - 1:
            print(f'  Bin {target_bin} Epoch {epoch}: train={avg_train_loss:.6f} val={avg_val_loss:.6f}')
        if epochs_without_improvement >= args.patience:
            break
    if best_model_state is not None:
        model.load_state_dict(best_model_state)
    ckpt = {'model_state_dict': model.state_dict(), 'best_val_loss': best_val_loss, 'Ps_mean': Ps_mean[target_bin].item(), 'Ps_std': Ps_std[target_bin].item(), 'Iplus_mean': Iplus_mean[target_bin].item(), 'Iplus_std': Iplus_std[target_bin].item(), 'Iminus_mean': Iminus_mean[target_bin].item(), 'Iminus_std': Iminus_std[target_bin].item(), 'bin_idx': target_bin, 'use_hidden': args.use_hidden, 'hidden_dim': args.hidden_dim}
    return (model, ckpt)

def augment_training_data_with_gap_fill(Ps_train, Iplus_train, Iminus_train, lookup_path, num_bins=500, num_fill_per_bin=5000):
    ...
    print(f'Gap-fill augmentation: {bins_filled}/{num_bins} bins had significant gaps. Added {num_fill_per_bin} synthetic samples -> total {aug_Ps.shape[0]}')
    return (aug_Ps, aug_Ip, aug_Im)

def parse_args():
    parser = argparse.ArgumentParser(description='Train 500 bin models and combine into a single model.')
    parser.add_argument('--output-dir', type=str, default='combined_500_results', help='Directory for combined model and outputs')
    parser.add_argument('--model-dir', type=str, default='single_bin_results', help='Directory with existing binning_model_bin_{idx}.pth checkpoints to resume/load')
    parser.add_argument('--data-dir', type=str, default='.', help='Directory containing binning_training.pkl and binning_testing.pkl')
    parser.add_argument('--num-bins', type=int, default=500)
    parser.add_argument('--max-bins', type=int, default=None, help='Max bins to train (for testing). Default: all 500')
    ...
    parser.add_argument('--min-delta', type=float, default=1e-06)
    parser.add_argument('--lr-factor', type=float, default=0.5)
    parser.add_argument('--lr-min', type=float, default=1e-06)
...
def main():
    ...
    test_dataset_full = data.TensorDataset(Ps_test_full, Iminus_test_full, Iplus_test_full)
    (val_dataset, test_dataset) = data.random_split(test_dataset_full, [0.5, 0.5])
    ...
    bin_models = []
    stats = {'Ps_mean': Ps_mean.numpy(), 'Ps_std': Ps_std.numpy(), 'Iplus_mean': Iplus_mean.numpy(), 'Iplus_std': Iplus_std.numpy(), 'Iminus_mean': Iminus_mean.numpy(), 'Iminus_std': Iminus_std.numpy()}
    ...
    for target_bin in range(max_bins):
        ...
        resume_from = None
        resume_candidates = [os.path.join(args.output_dir, f'binning_model_bin_{target_bin}.pth'), os.path.join(args.model_dir, f'binning_model_bin_{target_bin}.pth')]
        for candidate in resume_candidates:
            if os.path.exists(candidate):
                resume_from = candidate
                break
        (model, _) = train_single_bin(target_bin=target_bin, Ps_train_norm=Ps_train_norm, Iplus_train_norm=Iplus_train_norm, Iminus_train_norm=Iminus_train_norm, val_Ps_bin=val_Ps_bin, val_Iplus_bin=val_Iplus_bin, val_Iminus_bin=val_Iminus_bin, Ps_mean=Ps_mean, Ps_std=Ps_std, Iplus_mean=Iplus_mean, Iplus_std=Iplus_std, Iminus_mean=Iminus_mean, Iminus_std=Iminus_std, device=device, args=args, resume_checkpoint=resume_from)
        bin_models.append(model)
    for (bin_idx, model) in enumerate(bin_models[:max_bins]):
        out_path = os.path.join(args.output_dir, f'binning_model_bin_{bin_idx}.pth')
        torch.save({'model_state_dict': model.state_dict(), 'Ps_mean': stats['Ps_mean'][bin_idx], 'Ps_std': stats['Ps_std'][bin_idx], 'Iplus_mean': stats['Iplus_mean'][bin_idx], 'Iplus_std': stats['Iplus_std'][bin_idx], 'Iminus_mean': stats['Iminus_mean'][bin_idx], 'Iminus_std': stats['Iminus_std'][bin_idx], 'bin_idx': bin_idx, 'use_hidden': args.use_hidden, 'hidden_dim': args.hidden_dim}, out_path)
    ...
    payload = {'num_bins': num_bins, 'bin_state_dicts': [m.state_dict() for m in bin_models], 'stats': stats, 'use_hidden': getattr(args, 'use_hidden', True), 'hidden_dim': getattr(args, 'hidden_dim', 32)}
    ...
    if os.path.exists(test_path):
        ...
        rpe_iplus[mask_iplus] = np.abs(pred_iplus[mask_iplus] - Iplus_test[mask_iplus]) / np.abs(Iplus_test[mask_iplus]) * 100.0
        rpe_iminus[mask_iminus] = np.abs(pred_iminus[mask_iminus] - Iminus_test[mask_iminus]) / np.abs(Iminus_test[mask_iminus]) * 100.0
        print(f'Median RPE I+ (across all): {np.median(rpe_iplus[mask_iplus]):.4f}%')
        print(f'Median RPE I- (across all): {np.median(rpe_iminus[mask_iminus]):.4f}%')
    print('\nDone.')
