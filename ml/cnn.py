...
Usage:
  python ml/cnn.py --spectra ml/data/spectra_v6.npz --max-samples 20000 --epochs 30
  python ml/cnn.py --test-only --checkpoint ml/cnn_pq_results_v2/cnn_pq_best.pth
"""
import argparse
import json
import sys
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / 'cnn_pq_results_v4'
SEED = lstm_pq.SEED
...
def train_model(train_dataset, val_dataset, stats, *, channels, kernel_sizes, dropout, pool_bins, num_epochs, batch_size, learning_rate, patience, checkpoint_path=None, device=DEVICE):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val < best_val - MIN_DELTA:
            ...
            if checkpoint_path is not None:
                save_checkpoint(checkpoint_path, model=model, stats=stats, best_val_loss=best_val, best_epoch=best_epoch, channels=channels, kernel_sizes=kernel_sizes, dropout=dropout, pool_bins=pool_bins)
        # else:
        #     stale += 1
        #     if stale >= patience:
...
def main():
    ...
    parser.add_argument('--channels', type=parse_int_tuple, default=CHANNELS, help='Residual-block channel widths, comma-separated (default: 64,128,128,192)')
    parser.add_argument('--kernel-sizes', type=parse_int_tuple, default=KERNEL_SIZES, help='Odd conv kernel sizes, one per residual block (default: 7,5,3,3)')
