...
class BurnConfig:
    ...
    def __post_init__(self) -> None:
        self.lookup_path = REPO_ROOT / "Data_Creation" / "burn_lookup_table.pkl"
...
def load_burn_lookup(config: BurnConfig) -> tuple[pd.DataFrame, np.ndarray]:
    lookup_path = Path(config.lookup_path)
    burn_lookup_df = pd.read_pickle(lookup_path)
    lookup_burn_freqs = (
        burn_lookup_df.groupby("burn_bin_idx")["burn_freq"]
        .first()
        .sort_index()
        .to_numpy(dtype=float)
    )
    return burn_lookup_df, lookup_burn_freqs


def lookup_burn_bin_idx(f_value: float, lookup_burn_freqs: np.ndarray) -> int:
    return int(np.argmin(np.abs(lookup_burn_freqs - float(f_value))))


def build_mappers_for_bins(
    f: np.ndarray,
    polarization: float,
    burn_lookup_df: pd.DataFrame,
    lookup_burn_freqs: np.ndarray,
) -> list[BurnLookupMapper]:
    grouped = {
        burn_bin_idx: group
        for burn_bin_idx, group in burn_lookup_df.groupby("burn_bin_idx")
    }
    mappers: list[BurnLookupMapper] = []
    for bin_idx in tqdm.tqdm(range(len(f)), desc="Building mappers"):
        burn_bin_idx = lookup_burn_bin_idx(float(f[bin_idx]), lookup_burn_freqs)
        mapper = BurnLookupMapper(f, polarization, burn_bin_idx, grouped[burn_bin_idx])
...
def find_best_amp_for_bin(
    ...
    for amp in amp_values:
        ...
        ps_try, iplus_try, iminus_try = mapper.apply_bin_burn(
...
def main() -> None:
    ...
    burn_lookup_df, lookup_burn_freqs = load_burn_lookup(config)
    result = optimize_binwise_incremental(
        config, polarization, burn_lookup_df, lookup_burn_freqs
    )

    out_dir = REPO_ROOT / "results" / "current" / "binwise_incremental"
    out_dir.mkdir(parents=True, exist_ok=True)
    plot_greedy_burns(
        result, out_dir / f"incremental_policy_P{polarization:.2f}.png")

    print(f"Bin-wise optimization (one burn per bin) at P={polarization * 100:.2f}%:")
    ...
    print(f"Saved artifacts to {REPO_ROOT / 'results' / 'current' / 'binwise_incremental'}")
