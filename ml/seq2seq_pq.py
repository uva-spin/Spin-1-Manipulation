...
def prepare_datasets(arrays, *, val_frac=VAL_FRAC, test_frac=TEST_FRAC, seed=SEED, max_samples=None, noise_std=NOISE_STD):
    ...
    applied_power = np.asarray(arrays['applied_power']).reshape(-1)
    ...
    pwr_mean = applied_power[local_train].mean()
    pwr_std = applied_power[local_train].std()
...
def load_checkpoint(path, *, device=DEVICE):
    ...
    model = Seq2SeqPQModel(input_size=ckpt.get('input_size', ckpt['stats']['input_size']), hidden_size=ckpt['hidden_size'], num_layers=ckpt['num_layers'], dropout=ckpt.get('dropout', DROPOUT)).to(device)
...
def train_model(train_dataset, val_dataset, stats, *, hidden_size, num_layers, dropout, num_epochs, batch_size, learning_rate, patience, checkpoint_path=None, device=DEVICE):
    ...
    q_mean = stats['Q_mean']
    q_std = stats['Q_std']
    model = Seq2SeqPQModel(input_size=stats['input_size'], hidden_size=hidden_size, num_layers=num_layers, dropout=dropout).to(device)
...
@torch.no_grad()
def predict_denormalized(model, dataset, stats, *, batch_size=BATCH_SIZE, device=DEVICE):
...
@torch.no_grad()
def evaluate_model(model, dataset, stats, *, batch_size=BATCH_SIZE, device=DEVICE):
...
def save_example_signal_plots(arrays, stats, metrics, plots_dir, *, n_examples=N_EXAMPLE_PLOTS, seed=SEED):
    plots_dir.mkdir(parents=True, exist_ok=True)
    ...
    for (panel_i, local_i) in enumerate(pick):
        ...
        ax_s.plot(freq, ps, color='#1f2933', label='$P_s = I_+ + I_-$', lw=1.8)
        ax_s.plot(freq, ip, color='#2f6fed', label='$I_+$', alpha=0.75, ls='--', lw=1.2)
        ax_s.plot(freq, im, color='#c45c26', label='$I_-$', alpha=0.75, ls='--', lw=1.2)
        ax_s.plot(freq, q_spec, color='#0f766e', label='$Q = I_+ - I_-$', alpha=0.8, lw=1.2)
        ax_s.set_xlabel('Frequency offset')
        ax_s.set_ylabel('Amplitude')
        ax_s.set_title(f'Test #{gi} ({src})  ·  $p_0={p0_v:.3f}$  ·  P={power:.1e}  ·  steps={steps}')
        _apply_axes_style(ax_s)
        ax_s.legend(loc='upper right', fontsize=8, frameon=False)
        x_bars = np.arange(2)
        ax_pq.bar(x_bars - 0.2, [tp, tq], width=0.38, color='#94a3b8', edgecolor='white', label='True')
        ax_pq.bar(x_bars + 0.2, [pp, pq], width=0.38, color='#2f6fed', edgecolor='white', label='Pred')
        ax_pq.set_xticks(x_bars)
        ax_pq.set_xticklabels(['$P_{\\mathrm{total}}$', '$Q_{\\mathrm{total}}$'])
        ax_pq.set_title(f'$P$ RPE ${rpe_p:.2f}\\%$  ·  $Q$ RPE ${rpe_q:.2f}\\%$')
        ...
        if axes is not None and panel_i < summary_rows:
            ...
            ax_sum_s.plot(freq, ps, color='#1f2933', label='$P_s$', lw=1.6)
            ax_sum_s.plot(freq, q_spec, color='#0f766e', label='$Q$', alpha=0.8, ls='--', lw=1.1)
            ax_sum_s.set_title(f'#{gi} ({src})  ·  $p_0={p0_v:.3f}$')
            _apply_axes_style(ax_sum_s)
            if panel_i == 0:
                ax_sum_s.legend(fontsize=8, frameon=False)
            ax_sum_pq.bar(x_bars - 0.2, [tp, tq], width=0.38, color='#94a3b8', edgecolor='white', label='True')
            ax_sum_pq.bar(x_bars + 0.2, [pp, pq], width=0.38, color='#2f6fed', edgecolor='white', label='Pred')
            ax_sum_pq.set_xticks(x_bars)
            ax_sum_pq.set_xticklabels(['$P$', '$Q$'])
            ax_sum_pq.set_title(f'$P$ ${rpe_p:.2f}\\%$  ·  $Q$ ${rpe_q:.2f}\\%$')
...
def main():
    parser = argparse.ArgumentParser(description='Train/evaluate LSTM seq model: Ps spectrum → P_total, Q_total.')
    ...
    parser.add_argument('--checkpoint', type=Path, default=None, help='Checkpoint path (default: <out-dir>/seq2seq_pq_best.pth)')
    ...
    if args.test_only:
        ...
        best_val = ckpt.get('best_val_loss', 'nan')
    ...
    save_example_signal_plots(arrays, stats, metrics, args.out_dir, n_examples=args.n_examples)
