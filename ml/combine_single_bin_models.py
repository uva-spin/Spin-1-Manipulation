...
def _load_output_stats(model_payload, bin_idx):
    """Return per-bin (P_mean, P_std, Q_mean, Q_std) from a checkpoint."""
    if 'P_mean' in model_payload and 'Q_mean' in model_payload:
        return (model_payload['P_mean'], model_payload['P_std'], model_payload['Q_mean'], model_payload['Q_std'])
    if 'Iplus_mean' in model_payload and 'Iminus_mean' in model_payload:
        return (model_payload['Iplus_mean'], model_payload['Iplus_std'], model_payload['Iminus_mean'], model_payload['Iminus_std'])
    raise KeyError(f'Model file for bin {bin_idx} is missing output scaling stats (expected P_mean/P_std/Q_mean/Q_std or legacy Iplus/Iminus stats).')

def _output_stats_are_pq(model_payload):
...
def load_bin_models(model_dir, bin_indices):
    ...
    loaded_bin_indices = []
    target_mode = None
    pq_post_correct = None
    targets_precalibrated = None
    pq_target_scope = None
    use_hidden = None
    hidden_dim = None
    feature_names = None
    feature_set = None
    ...
    for bin_idx in bin_indices:
        ...
        payload_target_mode = 'pq' if _output_stats_are_pq(model_payload) else str(model_payload.get('target_mode', 'iplus_iminus'))
        _require_same(target_mode, payload_target_mode, 'target_mode', bin_idx)
        target_mode = payload_target_mode
        payload_pq_post_correct = model_payload.get('pq_post_correct', True)
        payload_targets_precalibrated = model_payload.get('targets_precalibrated', False)
        payload_pq_target_scope = str(model_payload.get('pq_target_scope', 'per_bin' if payload_targets_precalibrated else ''))
        if pq_post_correct is None:
            pq_post_correct = payload_pq_post_correct
        else:
            _require_same(pq_post_correct, payload_pq_post_correct, 'pq_post_correct', bin_idx)
        if targets_precalibrated is None:
            targets_precalibrated = payload_targets_precalibrated
        else:
            _require_same(targets_precalibrated, payload_targets_precalibrated, 'targets_precalibrated', bin_idx)
        if pq_target_scope is None:
            pq_target_scope = payload_pq_target_scope or None
        elif payload_pq_target_scope:
            _require_same(pq_target_scope, payload_pq_target_scope, 'pq_target_scope', bin_idx)
    ...
    stats = {'X_mean': np.stack(x_mean_rows, axis=0).astype(np.float32), 'X_std': np.stack(x_std_rows, axis=0).astype(np.float32), 'Ps_mean': np.asarray(ps_mean), 'Ps_std': np.asarray(ps_std), 'P_mean': np.asarray(p_mean), 'P_std': np.asarray(p_std), 'Q_mean': np.asarray(q_mean), 'Q_std': np.asarray(q_std)}
    ...
    return {'bin_state_dicts': bin_state_dicts, 'stats': stats, 'loaded_bin_indices': loaded_bin_indices, 'target_mode': target_mode if target_mode is not None else 'pq', 'pq_post_correct': pq_post_correct if pq_post_correct is not None else True, 'targets_precalibrated': targets_precalibrated if targets_precalibrated is not None else False, 'pq_target_scope': pq_target_scope or 'per_bin', 'use_hidden': use_hidden if use_hidden is not None else True, 'hidden_dim': hidden_dim if hidden_dim is not None else 256, 'feature_names': feature_names if feature_names is not None else list(DEFAULT_FEATURE_NAMES), 'feature_set': feature_set if feature_set is not None else 'ps', 'input_dim': input_dim if input_dim is not None else 1, 'best_val_loss': np.asarray(best_val_loss), 'metrics_per_bin': metrics_per_bin}
...
def main():
    ...
    payload = {'num_bins': len(combined['bin_state_dicts']), 'bin_state_dicts': combined['bin_state_dicts'], 'stats': combined['stats'], 'use_hidden': combined['use_hidden'], 'hidden_dim': combined['hidden_dim'], 'input_dim': combined['input_dim'], 'feature_names': combined['feature_names'], 'feature_set': combined['feature_set'], 'target_mode': combined.get('target_mode', 'pq'), 'pq_post_correct': combined.get('pq_post_correct', True), 'pq_target_scope': combined.get('pq_target_scope', 'per_bin'), 'targets_precalibrated': combined.get('targets_precalibrated', False), 'loaded_bin_indices': combined['loaded_bin_indices'], 'best_val_loss_per_bin': combined['best_val_loss'], 'metrics_per_bin': combined['metrics_per_bin'], 'source_model_dir': str(args.model_dir), 'dataset_note': 'Per-bin models trained on combined_train_all NPZs (ssrf + afp + unmanipulated); features from single_bin.py'}
