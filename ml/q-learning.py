...
class BurnConfig:
    ...
    def amp_values(self):
        ...
        if self.n_amp_bins <= 1:
            return np.array([0.0])
        if self.amp_min <= 0.0:
            positive = np.linspace(self.amp_max / (self.n_amp_bins - 1), self.amp_max, self.n_amp_bins - 1)
        else:
            positive = np.linspace(self.amp_min, self.amp_max, self.n_amp_bins - 1)
...
class TensorBurnEnv:
    ...
    def _action_to_burn(self, action):
        if self.config.enforce_full_spectrum:
            ...
            x_idx = action // self.config.n_amp_bins
            amp_idx = action % self.config.n_amp_bins
        x = self.config.x_values[x_idx]
        amp = self.config.amp_values[amp_idx]
        return (x_idx, x, amp)
    ...
    def reset(self, polarization=None):
        ...
        self._iplus = np.asarray(iplus)
        self._iminus = np.asarray(iminus)
    ...
    def step(self, action):
        ...
        if not self.config.enforce_full_spectrum and x_idx in self._used_x_bins:
            return (self.state_index(), -1e-06, False, {'repeated_x_bin': True, 'bin_idx': x_idx, 'x': x, 'f': x, 'amp': amp, 'q': self._q, 'q_gain': self._q - self._q0})
        ...
        if amp <= 0.0:
            ...
            return (self.state_index(), 0.0, done, {'skipped': True, 'bin_idx': x_idx, 'x': x, 'f': x, 'freq_bin_idx': freq_bin_idx, 'amp': amp, 'q': self._q, 'q_gain': self._q - self._q0})
        ps = self._ps.copy()
        ...
        info = {'bin_idx': x_idx, 'x': x, 'f': x, 'freq_bin_idx': freq_bin_idx, 'amp': amp, 'q': self._q, 'q_gain': self._q - self._q0}
...
def _read_lookup_pickle(path):
    ...
    try:
        ...
        if 'numpy._core' not in str(exc):
            raise
        module_remap = {'numpy._core': 'numpy.core', 'numpy._core.multiarray': 'numpy.core.multiarray', 'numpy._core.numeric': 'numpy.core.numeric'}
...
def load_mapper(config):
    ...
    if not lookup_path.is_file():
        raise FileNotFoundError(f'Lookup table not found at {lookup_path}. Run Data_Creation/lookup_table.py first.')
    mapping_data = _read_lookup_pickle(lookup_path)
    mapper = ssRFMapper(config.f, config.sigma, config.gamma, x0=0.0, amp=0.001)
...
class QLearningAgent:
    ...
    def __init__(self, n_states, n_actions, alpha=0.15, gamma=0.99, epsilon=1.0, optimistic_init=0.02, seed=42):
        ...
        init = optimistic_init
        self.q_table_a = np.full((n_states, n_actions), init)
        self.q_table_b = np.full((n_states, n_actions), init)
        ...
        if done:
            ...
            if next_action_mask is not None:
                ...
                if not np.any(mask):
                    ...
                    greedy_action = np.argmax(next_values)
                    best_next = eval_table[next_state, greedy_action]
            else:
                greedy_action = np.argmax(next_values)
                best_next = eval_table[next_state, greedy_action]
        td_target = reward + self.gamma * best_next
        target[state, action] += self.alpha * (td_target - target[state, action])
...
def epsilon_schedule(episode, total_episodes, eps_start=1.0, eps_end=0.02):
    if total_episodes <= 1:
        return eps_end
...
def train(config, episodes=5000, polarizations=None, seed=0, eps_start=1.0, eps_end=0.02):
    ...
    for ep in tqdm.tqdm(range(episodes), desc='Training Q-learning agent'):
        ...
        for _ in range(config.max_burns):
            ...
            agent.update(state, action, reward, next_state, done, next_action_mask=next_mask)
            ep_return += reward
            state = next_state
            if done:
                break
        history.append(ep_return)
    stats = {'q_lo': q_lo, 'q_hi': q_hi, 'p_lo': np.min(polarizations), 'p_hi': np.max(polarizations), 'final_q_gain': env.current_q - env.initial_q, 'episode_returns': np.asarray(history)}
...
def greedy_episode(env, agent, polarization):
    ...
    for step in range(env.config.max_burns):
        ...
        trace.append({'step': step + 1, 'action': action, 'bin_idx': info['bin_idx'], 'f': info['f'], 'amp': info['amp'], 'reward': reward, 'q': info['q'], 'q_gain': info['q_gain']})
        state = next_state
        if done:
            break
...
def main():
    ...
    parser.add_argument('--free-bin-selection', action='store_true', help='If set, action chooses both bin and amplitude (legacy mode). Default is full-spectrum mode: one step per bin, action chooses amplitude only.')
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--output-dir', type=Path, default=REPO_ROOT / 'results' / 'current' / 'q_learning')
    args = parser.parse_args()
    config = BurnConfig(max_burns=args.max_burns, enforce_full_spectrum=not args.free_bin_selection)
    polarizations = np.linspace(0.4, 0.5, 20)
    print('Training Q-learning agent for tensor Q polarization...')
    (agent, env, stats) = train(config, episodes=args.episodes, polarizations=polarizations, seed=args.seed)
    ...
    print(f"Q bounds: [{stats['q_lo']:.4f}, {stats['q_hi']:.4f}]  P bounds: [{stats['p_lo']:.4f}, {stats['p_hi']:.4f}]")
    print(f'Greedy policy at P={eval_p * 100:.2f}%:')
    for row in greedy['trace']:
        if row['action'] is None:
            print(f"  start: Q={row['q'] * 100:.5f}%")
        else:
            print(f"  burn {row['step']}: bin={row['bin_idx']}, f={row['f']:.3f}, amp={row['amp']:.4e}, reward={row['reward']:.5f}, Q={row['q'] * 100:.5f}%")
