...
class DenoisingAutoencoder(nn.Module):
    ...
    def __init__(self, input_dim=500, hidden_dims=(256, 128, 64)):
        super().__init__()
...
def select_examples_by_polarization(P, scales, rng):
    """
...
def select_examples_by_burn_intensity(clean, rng, n=4, sigma=15.0):
    """
    ...
    return [(idx, f'burn (intensity={burn_scores[idx]:.3f})') for idx in chosen]

def _minmax_scale_01(x):
...
def resolve_spectra_path(spectra=None):
    ...
    raise FileNotFoundError('Spectra NPZ not found. Pass --spectra PATH. Tried: ' + tried)

def load_voigt_burn_ps(spectra=None, num_points=500):
    ...
    try:
        ...
        spectra_arr = np.asarray(loaded['spectra'])
        p_total = np.asarray(loaded['P_total']).reshape(-1) if 'P_total' in loaded.files else None
    finally:
        loaded.close()
    if spectra_arr.ndim != 3 or spectra_arr.shape[1] != 2 or spectra_arr.shape[2] != num_points:
        raise ValueError(f'{path}: expected spectra shape (N, 2, {num_points}), got {spectra_arr.shape}')
    iplus = spectra_arr[:, 0, :]
    ...
    if p_total is not None and p_total.shape[0] == ps.shape[0]:
        ...
        p_proxy = np.divide(diff, total, out=np.zeros_like(diff), where=np.abs(total) > 1e-12)
        p_src = 'I+/I- proxy'
    print(f'Loaded Voigt-burn spectra: {path}  N={ps.shape[0]}  Ps shape={ps.shape} (from Iplus/Iminus; P from {p_src})', flush=True)
    return (ps, p_proxy.astype(np.float32, copy=False))

def load_and_prepare_data(spectra=None, num_points=500, noise_std=0.1, val_frac=0.2, test_frac=0.2, seed=42, scale_01=True, override_stats=None):
    """
    ...
    (ps_all, p_all) = load_voigt_burn_ps(spectra=spectra, num_points=num_points)
    n_total = ps_all.shape[0]
    if n_total < 3:
    ...
    print(f'Split: train_pool={len(Ps_train)}  test={len(Ps_test)}  (val carved from train_pool)', flush=True)
    if scale_01:
    ...
    n_val = n_train * val_frac
    val_idx = perm[:n_val]
    train_idx = perm[n_val:]
    train_dataset = data.TensorDataset(torch.from_numpy(Ps_train_noisy_norm[train_idx]).float(), torch.from_numpy(Ps_train_norm[train_idx]).float())
    val_dataset = data.TensorDataset(torch.from_numpy(Ps_train_noisy_norm[val_idx]).float(), torch.from_numpy(Ps_train_norm[val_idx]).float())
    test_dataset = data.TensorDataset(torch.from_numpy(Ps_test_noisy_norm).float(), torch.from_numpy(Ps_test_norm).float())
...
def _publication_style():
    ...
    plt.rcParams.update({'font.family': 'serif', 'font.serif': ['Times New Roman', 'DejaVu Serif', 'serif'], 'font.size': 11, 'axes.labelsize': 12, 'axes.titlesize': 12, 'xtick.labelsize': 10, 'ytick.labelsize': 10, 'legend.fontsize': 10, 'figure.dpi': 150, 'savefig.dpi': 300, 'savefig.bbox': 'tight', 'axes.linewidth': 1.2, 'lines.linewidth': 1.5, 'lines.markersize': 4})

def run_test(model, test_dataset, stats, device, output_dir, noise_std, P_test, seed=42, n_examples=5, max_heatmap_samples=200):
    """
    ...
    stats_dict = {'n_samples': n_samples, 'n_bins': n_bins, 'noise_std': noise_std, 'MSE_normalized': mse_norm, 'MSE_raw': mse_raw, 'MAE_normalized': mae_norm, 'MAE_raw': mae_raw, 'RMSE_raw': rmse_raw, 'MSE_noisy_vs_clean_raw': mse_noisy_raw, 'MAE_noisy_vs_clean_raw': mae_noisy_raw, 'MSE_improvement_pct': mse_improvement_pct, 'MAE_improvement_pct': mae_improvement_pct, 'residual_mean': residual_mean, 'residual_std': residual_std, 'per_bin_MAE': {'median': np.median(per_bin_mae), 'mean': np.mean(per_bin_mae), 'min': np.min(per_bin_mae), 'max': np.max(per_bin_mae)}}
    ...
    polarization_scales = [(5, 0.03, 0.08), (10, 0.08, 0.12), (20, 0.15, 0.25), (30, 0.25, 0.35), (40, 0.35, 0.45), (50, 0.45, 0.55)]
    rng = np.random.default_rng(seed)
    pol_selections = select_examples_by_polarization(P_test, polarization_scales, rng)
    n_burn = min(10, max(6, n_examples - len(polarization_scales)))
    burn_selections = select_examples_by_burn_intensity(clean_denorm, rng, n=n_burn, sigma=5.0)
    seen = set()
    example_selections = []
    ...
    plt.tight_layout()
    for fmt in ('png', 'pdf'):
        fig.savefig(os.path.join(plots_dir, f'lineshape_examples.{fmt}'), dpi=600 if fmt == 'png' else None, bbox_inches='tight')
    plt.close()
    ...
    if burn_only:
        ...
        plt.tight_layout()
        for fmt in ('png', 'pdf'):
            fig_b.savefig(os.path.join(plots_dir, f'lineshape_examples_burns.{fmt}'), dpi=600 if fmt == 'png' else None, bbox_inches='tight')
        plt.close()
    heatmap_n = min(max_heatmap_samples, n_samples)
    fig2 = plt.figure(figsize=(8, 4))
    plt.imshow(residuals[:heatmap_n], aspect='auto', cmap='coolwarm')
    ...
    plt.tight_layout()
    for fmt in ('png', 'pdf'):
        fig2.savefig(os.path.join(plots_dir, f'residuals_heatmap.{fmt}'), dpi=600 if fmt == 'png' else None, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    for fmt in ('png', 'pdf'):
        fig3.savefig(os.path.join(plots_dir, f'residuals_hist.{fmt}'), dpi=600 if fmt == 'png' else None, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    for fmt in ('png', 'pdf'):
        fig4.savefig(os.path.join(plots_dir, f'per_bin_mae.{fmt}'), dpi=600 if fmt == 'png' else None, bbox_inches='tight')
    plt.close()
    ...
    lims = [min(flat_clean.min(), flat_pred.min()), max(flat_clean.max(), flat_pred.max())]
    ...
    plt.tight_layout()
    for fmt in ('png', 'pdf'):
        fig5.savefig(os.path.join(plots_dir, f'pred_vs_clean.{fmt}'), dpi=600 if fmt == 'png' else None, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    for fmt in ('png', 'pdf'):
        fig6.savefig(os.path.join(plots_dir, f'dae_example.{fmt}'), dpi=600 if fmt == 'png' else None, bbox_inches='tight')
    plt.close()
    print(f'\nPlots saved to {plots_dir}/ (individual examples + summary figures)')

def parse_args():
    parser = argparse.ArgumentParser(description='Train DAE to denoise 500-point NMR signals.')
    parser.add_argument('--spectra', type=str, default=None, help='Path to spectra.npz (same file as spectrum_pq.py). Default: search Data_Creation/ then ml/ dae_voigt_burn_spectra/')
    parser.add_argument('--test-frac', type=float, default=0.2, help='Fraction of spectra held out as test (default: 0.2)')
    parser.add_argument('--output-dir', type=str, default=str(DEFAULT_OUTPUT_DIR), help=f'Directory for model and plots (default: {DEFAULT_OUTPUT_DIR})')
    parser.add_argument('--noise-std', type=float, default=0.3, help='Noise level (fraction of signal std). Applied before scaling.')
    parser.add_argument('--no-scale-01', action='store_true', help='Disable min-max scaling to [0,1]. Use z-score normalization instead.')
    ...
    parser.add_argument('--examples', type=int, default=12, help='Max example plots (polarization + hole-burning)')
    parser.add_argument('--max-heatmap-samples', type=int, default=200, help='Max samples for residual heatmap')
    parser.add_argument('--test-only', action='store_true', help='Skip training; load checkpoint and run evaluation/plots only.')
    parser.add_argument('--checkpoint', type=str, default=None, help='Path to .pth checkpoint (default: <output-dir>/dae_denoise_500.pth). Required for --test-only.')
...
def main():
    ...
    if args.test_only:
        ...
        if not os.path.isfile(ckpt_path):
            raise FileNotFoundError(f'Checkpoint not found: {ckpt_path}. Train first or set --checkpoint.')
        ...
        (train_dataset, val_dataset, test_dataset, stats, P_test) = load_and_prepare_data(spectra=args.spectra, noise_std=noise_std_ckpt, seed=args.seed, scale_01=scale_01_ckpt, override_stats=ckpt_stats if not scale_01_ckpt else None, test_frac=args.test_frac)
        hidden_dims = tuple(ckpt['hidden_dims'])
        model = DenoisingAutoencoder(input_dim=500, hidden_dims=hidden_dims).to(device)
        model.load_state_dict(ckpt['model_state_dict'])
        print(f'Loaded checkpoint: {ckpt_path}')
        run_test(model=model, test_dataset=test_dataset, stats=stats, device=device, output_dir=args.output_dir, noise_std=noise_std_ckpt, P_test=P_test, seed=args.seed, n_examples=args.examples, max_heatmap_samples=args.max_heatmap_samples)
        print('Done.')
        return
    print(f'Noise std (fraction of signal): {args.noise_std}')
    print(f'Scale to [0,1]: {not args.no_scale_01}')
    (train_dataset, val_dataset, test_dataset, stats, P_test) = load_and_prepare_data(spectra=args.spectra, noise_std=args.noise_std, seed=args.seed, scale_01=not args.no_scale_01, test_frac=args.test_frac)
    train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
    val_loader = data.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False)
    test_loader = data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False)
    model = DenoisingAutoencoder(input_dim=500, hidden_dims=tuple(args.hidden_dims)).to(device)
    ...
    torch.save({'model_state_dict': model.state_dict(), 'stats': stats, 'hidden_dims': args.hidden_dims, 'noise_std': args.noise_std}, os.path.join(args.output_dir, 'dae_denoise_500.pth'))
    print(f'Saved model to {args.output_dir}/dae_denoise_500.pth')
    run_test(model=model, test_dataset=test_dataset, stats=stats, device=device, output_dir=args.output_dir, noise_std=args.noise_std, P_test=P_test, seed=args.seed, n_examples=args.examples, max_heatmap_samples=args.max_heatmap_samples)
    print('Done.')
